...
if [ -z "${BACKEND_DRIVER:-}" ] || [ ! -x "${BACKEND_DRIVER:-}" ]; then
  echo "[verify_std_crypto_tls_smoke] missing backend driver under CHENG_ROOT=$CHENG_ROOT" 1>&2
  echo "[verify_std_crypto_tls_smoke] hint: build backend_mvp_driver first" 1>&2
  exit 1
fi
