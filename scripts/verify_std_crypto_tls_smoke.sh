...
if [ -z "${BACKEND_DRIVER:-}" ] || [ ! -x "${BACKEND_DRIVER:-}" ]; then
  echo "[verify_std_crypto_tls_smoke] missing backend driver under ROOT=$ROOT" 1>&2
  echo "[verify_std_crypto_tls_smoke] hint: build artifacts/backend_driver/cheng first" 1>&2
  exit 1
fi

entry="$ROOT/src/tests/std_crypto_tls_smoke.cheng"
if [ ! -f "$entry" ]; then
  echo "[verify_std_crypto_tls_smoke] missing test entry: $entry" 1>&2
  exit 1
fi

out_dir="$ROOT/artifacts/verify/std_crypto_tls_smoke"
mkdir -p "$out_dir"
out_path="$out_dir/std_crypto_tls_smoke"
...
"$out_path"
echo "[verify_std_crypto_tls_smoke] ok"
