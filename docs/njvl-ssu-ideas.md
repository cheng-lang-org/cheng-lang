...
fn newUseFor*(v: var UseVersionTab, symId: SymId) =
...
fn getUseVersion*(v: UseVersionTab, symId: SymId): int =
  v.currentUseVersion.getOrDefault(symId, -1)
```

### VL Pass Modifications

In `vl.cheng`, modify `trExpr` to track uses:

```cheng
fn trExpr(c: var Context; dest: var TokenBuf; n: var Cursor) =
  case n.kind
  of Symbol:
    let s = n.symId
...
```cheng
fn trIte(c: var Context; dest: var TokenBuf; n: var Cursor) =
  # ... existing code ...
...
With SSU, the `duplifier.cheng` pass can be significantly simplified. Currently, it performs complex `isLastRead` analysis to determine when to use moves vs `=dup` calls.
