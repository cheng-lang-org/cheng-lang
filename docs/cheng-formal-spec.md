...
- `perf_memory_contract_smoke` 是当前正式性能/内存 smoke；`orc_perf_contract` 记录 ORC retain/release 与 alloc/free/live 配平，`*_compile_exec_phase_summary` 记录正式编译 phase 摘要，`*_compile_gap_breakdown` 记录 planner 之外的 object materialize/native link/line-map 真耗时，`crypto_hot_kernel_contract` 记录 SHA-256、X25519 pubkey、P-256 pubkey、P-256 sign 的热核 ns/op。
