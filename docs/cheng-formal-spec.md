...
- Dev 快速自举管线（host-only）：
  - 默认 profile：`BACKEND_FAST_DEV_PROFILE=1`（fast selfhost 默认注入）。
...
- driver 解析口径：`backend_driver_path` 默认优先稳定 driver `artifacts/backend_driver/cheng`，并带 stage0 编译探针；当首选不健康时按显式候选顺序尝试 `artifacts/backend_driver/cheng -> artifacts/backend_seed/cheng.stage2 -> artifacts/backend_selfhost_self_obj/cheng.stage2 -> ./cheng`，并跳过命中 `UE/UEs` 的候选；全部失败才阻断。
