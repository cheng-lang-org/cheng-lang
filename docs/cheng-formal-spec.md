...
- 运行时语义：`memRetain/memRelease` 仅改 refcount；refcount==0 立即释放并从管理链表中移除。**RC 默认原子实现**（可用 `CHENG_MM_ATOMIC=0` 关闭以换取性能）；跨线程共享仍需 `share_mt`/`Arc[T]` 与 `Send/Sync` 边界检查。
- 循环引用最佳实践：**显式打断**（如手动置 `nil`/拆分容器/断开引用），不内建 tracing GC。
- FFI 边界：跨 FFI 传递所有权需显式 `memRetain/memRelease`，避免悬垂指针。
- 可观测性：运行时用 `CHENG_MM_DIAG=1` 输出 retain/release 日志；计数器 API：`memRetainCount/memReleaseCount`（可用 `memDiagReset()` 清零）。编译期 ownership 诊断使用 `CHENG_OWNERSHIP_DIAGS=1`。
...
- 实现状态（2026-02-09）：stage1 C 后端已移除；生产链路使用后端 driver 直出 `.o/.exe`。moveHint/retain/release 的最终绑定以当前后端实现与回归脚本为准。
- 运行时闭环：`memRelease` 在 refcount==0 时立即释放并从管理链表摘除。
- 验收与可观测性：`examples/test_orc_closedloop.cheng` 覆盖 overwrite/return-move/`defer`/loop/if use-after 等场景；配合 `CHENG_MM_DIAG=1` 与计数器一致性检查。
...
- Codegen 注入：stage1 C 后端已移除；生产链路由后端 driver 直接生成 `.o/.exe`（见 `src/backend/tooling/backend_driver.cheng`）。Ownership 的最终落地位置以当前实现为准。
...
- 开关与诊断：`src/stage1/frontend_lib.cheng` 按 `CHENG_MM`/`CHENG_MM_STRICT` 开启 ownership/ORC；编译期诊断用 `CHENG_OWNERSHIP_DIAGS`；`CHENG_STAGE1_OWNERSHIP_DEEP=1` 强制深度遍历（默认浅遍历以提升 stage1 性能，仅用于对照/排障）；运行时计数与日志用 `CHENG_MM_DIAG`。
- 确定性与回归：`src/tooling/bootstrap.sh --fullspec` 与 `src/tooling/bootstrap_pure.sh --fullspec` 用于输出 hash 一致性检查；`examples/test_orc_closedloop.cheng` 作为 ORC 回归用例。
...
- 缓存键必须包含：编译器/标准库版本或 hash、`CHENG_MM`/`CHENG_MM_STRICT`、目标平台、规范版本。
- 建议使用 `CHENG_COMPILER_VERSION`/`CHENG_STAGE1_VERSION` 与 `CHENG_TARGET` 显式传入版本与目标平台标识，写入缓存键与产物元数据。
- CI/发布构建建议使用独立 `CHENG_CACHE_DIR` 或 `CHENG_CACHE=0`，避免跨版本污染。
- 产物元数据需记录：manifest+lock 版本、编译器版本、`CHENG_MM` 配置与 determinism hash。
- determinism hash 输入集合至少包含：backend driver/seed 版本、编译参数、目标平台、版本标识（`CHENG_COMPILER_VERSION`/`CHENG_STAGE1_VERSION`/`CHENG_TARGET`）、关键环境变量（`CHENG_MM`/`CHENG_MM_STRICT`/`CHENG_STAGE1_OWNERSHIP_DEEP`/`CHENG_CACHE`/`CHENG_IDE_ROOT`/`CHENG_GUI_ROOT` 等）与 manifest/lock 内容（如 `CHENG_PKG_LOCK`）。
...
- 建议基准：统计 retain/release 总量、escape 计数与编译耗时；对比 `CHENG_MM_STRICT=0/1` 差异作为优化回归阈值。
- 性能剖析：`CHENG_STAGE1_PROFILE=1` 输出 sem/mono/ownership/cgen 的统计与 top-k 节点耗时，用于定位 determinism/自举热点。
...
- 运行时：新增原子 RC 路径并与现有 ORC 共存；诊断开关复用 `CHENG_MM_DIAG` 记录原子 retain/release。
...
`bin/cheng-stage0 --mode:hrt`、`CHENG_ASM_RT`、`CHENG_HRT_PROFILE` 不再属于现行支持范围。  
历史约束文档（如 `docs/hrt-profile-v1.md`）仅保留为存档，不再作为生产规范与验收门禁。

### 0.5 直出 C 后端 Profile（C-Profile）

直出 C 是**编译后端 Profile**，不改变语言语法；通过降级/重写与运行时 shim 将语义稳定映射到 C。详细规范见 `docs/cheng-direct-c-profile.md`。

- 生产入口：统一使用 `cheng`，仅 `--emit=obj|exe`。
- 生产构建主链路仅保留 `obj/exe` 产物模式。
- stage1 FULLC 支持泛型单态化与 trait/concept 调度（vtable）。
- 诊断格式：`[C-Profile] not-mapped|unsupported: <feature> (FULLC.<Axis>-NN)`，Axis ∈ `L/G/D/A/F/R/T/V`。
- 历史开关不再纳入生产默认与验收门禁。

### 0.5.1 设计与实现注意（工程约束）

- `stage1` 为 FULLC 主路径；不支持的语义必须在前端**早诊断**并给出可行降级提示。
- not-mapped/unsupported 的诊断格式与 rewrite hint 以 `docs/cheng-direct-c-profile.md` 为准，避免文档/实现分叉。
- FFI/ABI 需强约束：`importc` 聚合返回走 wrapper/out 参数；`varargs` 仅允许受限签名并显式标注（自研后端 darwin/arm64 已回归 `importc` 直连 i32 子集；跨平台建议使用 wrapper 形态）。
  - `varargs` 调用点实现约束：显式构建栈参数区并保持 16B 对齐。
  - x86_64（SysV 子集）：`varargs` 调用前显式设置 `AL=0`（向量寄存器参数计数）。
  - AArch64 Darwin：命名溢出参数与变参统一按 8B 槽传递；`varargs[int32]` 写栈前做符号扩展。
- 默认初始化规则固定：带类型标注且省略初始化使用隐式默认值；`default[T]` 仅用于表达式位置，禁止用于 `let/var/const` 绑定初始化；`TypeExpr(expr)` 仅作类型转换，**不存在** `TypeExpr()` 默认初始化语法。
- 运行时/ABI 变更需配套 fullspec 回归与 determinism 校验，确保可复现与兼容性稳定。
- determinism 默认复用 bootstrap 输出进行单次 stage1 复跑（`CHENG_BOOTSTRAP_DET_REUSE_BOOTSTRAP=1`）；如需严格双跑可设为 `0`。
- 控制流表达式（`if/when/case` 作为表达式）在 C-Profile 中会降级为“临时变量 + 分支赋值”，要求类型可推导或显式标注。
- `&&`/`||` 的短路语义在降级时必须保持，右侧含控制流表达式仅在需要时求值。
- 诊断上下文可选：设置 `CHENG_DIAG_CONTEXT=1` 输出报错行与列指示，便于定位。

### 0.5.2 匿名函数与函数指针（语义差异）
...
- **C-Profile 降级**：提供 lowering 可视化/调试开关，合并临时变量减少 glue 代码；对 closure env 结构给出 size 报告与膨胀诊断。
- **构建确定性**：统一产物 metadata schema，强制记录 `CHENG_MM/CHENG_MM_STRICT/profile/target/seed`；提供缓存命中统计与失效原因。

已落地（v0.6.x 同步项）：
- **cgen 热路径缓存**：`cgenStripSpaces`/`cgenMangle` 结果缓存并在 profile 输出命中统计，减少重复字符串处理开销。
- **C-Profile 索引**：对 fn/type/iterator/wrapper/局部值引入哈希索引，避免线性查找导致的编译期放大。
...
- 当前没有独立的 slice/borrow-view 类型（`openArray` 已移除）；需要零拷贝时建议显式传 `ptr + len`（或封装为 tuple/object）。
...
- 实现约束：编译器在 `stage1` parser 与后端 `mvp` 类型 lowering 均会显式拒绝 `string`（报错提示改用 `str`/`cstring`）。
...
- 当需要移除/替换源码层语法（例如 `seq[T]` -> `T[]`、`@[...]` -> `[...]`、引入列表生成式等）时，推荐按以下最小流程落地，避免自举/文档/门禁漂移：
  1. 规范与文档先行：先更新本规范与相关设计文档（如 `docs/container-refactor.md`、`docs/list-comprehension.md`），并同步 `docs/cheng-skill/` 与 `$HOME/.codex/skills/cheng语言/`；跑 `sh src/tooling/verify_cheng_skill_consistency.sh`。
  ...
  4. 自举兼容：若 seed stage0 不支持新语法，通过 `scripts/gen_stage0_compat_src.py` 生成 `chengcache/stage0_compat/` overlay（仅用于 stage0 编译），overlay 中可做最小 rewrite（如 `T[]` -> `seq[T]`、`[]` -> `@[]`）；stage1/stage2 与日常编译必须回到新语法源树编译。
  5. 回归与 seed 更新：补最小正/反例 tests；跑 `sh verify.sh` 或相关 gate；需要刷新 seed 时用 `sh src/tooling/bootstrap_pure.sh` 并设置 `CHENG_BOOTSTRAP_UPDATE_SEED=1`。
...
- 位移位数：设被移位值位宽为 `W`（例如 `int32` 为 32；`int`/`uint` 为 `CHENG_INTBITS`），实际位移量 `k = rhs mod W`（结果落在 `[0, W-1]`）。
...
- 导入仅接受归一化模块路径（例如 `import cheng/<pkg>/...`、`import std/...`、`import ide/...`、`import ide/gui/...`），不接受字符串/绝对/相对路径；路径中**不允许空格**。
- **域名内容寻址（IPNS 风格）**：`cheng/<pkg>/...` 视为包域名，等价于 `pkg://cheng/<pkg>`；解析由 registry/lock 固定 `cid`，本地从 `chengcache/packages/<cid>/` 加载。
- **包内模块根**：对已解析到的包根目录（含 `cheng-package.toml`），源码以 `src/` 为模块根：`import cheng/<pkg>/<path>` 映射为 `<pkgroot>/src/<path>.cheng`（`<pkg>` 只用于选包，不参与包内路径）。
- 包搜索根：优先 `CHENG_PKG_ROOT/CHENG_PKG_ROOTS`；未设置时默认 `~/.cheng-packages`，并优先尝试 `cheng-<pkg>` 容器目录。
- 标准库导入规范：业务/编译器代码统一使用 `std/<path>` 入口，并映射到仓库 `src/std/<path>.cheng`；`cheng/stdlib/bootstrap/*` 已移除，不作为可用导入路径。
- 支持相同前缀合并：`import cheng/libp2p/[crypto,transport,swarm]`（方括号内用逗号分隔，路径整体不含空格）。
...
## 附录 A 直出 C Profile 约束摘要

本附录整理 `docs/cheng-direct-c-profile.md` 的核心约束，便于规范引用；完整细节与矩阵任务以该文档为准。

- 主路径：生产/验收统一使用后端直出（`chengc.sh --backend:obj`，默认即该路径）。
- 语义降级：`case/when/of`、控制流表达式、具名/默认参数、`?`、`defer`、`for-in`、`lambda`、`async/await` 等必须在前端 lowering 为显式控制流/状态机/包装函数；未降级能力需报 `[C-Profile] not-mapped|unsupported`。
- ABI 约束：`importc` 聚合返回改用 wrapper/out 参数；`varargs` 仅允许固定前缀参数 + 受控标量/指针（darwin/arm64 已回归直连 i32 子集，其它平台建议 wrapper）；捕获型回调必须显式 `ctx: void*` 传递环境。
  - `varargs` 调用点要求：16B 栈对齐；x86_64（SysV 子集）调用前写 `AL=0`；AArch64 Darwin 使用 8B 栈槽并对 `varargs[int32]` 执行符号扩展后入槽。
- 类型与布局：`int/uint` 映射 `NI/NU`（`CHENG_INTBITS` 决定）；`enum` v0 固定 `NI32`；`tuple`/`object` 直出为 `struct`/tagged union；`seq/str/slice` 依赖运行时 shim 的 ABI 约定。
- 符号命名：`importc` 符号不做 mangling；其余符号按 C‑Profile 规则编码（`.`→`_`，`_`→`Q_` 等）。
- 诊断与矩阵：每个降级/缺口必须携带任务 ID（例如 `FULLC.*`）与 rewrite hint，禁止静默生成不安全代码。

## 附录 B 自研后端与 IR v1 摘要

本附录记录自研后端落地的实现约束与 IR v1 摘要；完整任务清单与文件结构见 `docs/cheng-backend-arch.md`。

- 后端定位：保持语言语义不变，新增编译器内部 IR（HIR/MIR/SSA/LIR）以支撑优化与跨平台后端。
- IR 语义显式化：整数运算与移位使用 `sdiv/udiv`、`smod/umod`、`lshr/ashr` 等显式操作，避免 C 语义歧义。
- 产物策略：并行编译生成 `.o`，再统一链接为可执行文件；首发平台为 macOS ARM64。
