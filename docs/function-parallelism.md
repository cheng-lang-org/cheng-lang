...
| `UirFnTask`/`UirFnTaskResult` 合同 | 最小模型已落地 | `src/core/ir/function_task.cheng`、`src/tests/function_task_contract_smoke.cheng` | 已新增 UIR 专用任务/结果/只读快照数据模型；task 只保存标量索引、声明序、source span、成本、泛型 epoch、entry flag 和 owner module path，当前仍保持 `serial + job_count=1` 合同报告。 | `artifacts/backend_driver/cheng system-link-exec --root:/Users/lbcheng/cheng-lang --in:src/tests/function_task_contract_smoke.cheng --emit:exe --target:arm64-apple-darwin --out:/tmp/function_task_contract_smoke && /tmp/function_task_contract_smoke` | `UirFnTask` 不含 `BodyIR`、AST、函数体文本或共享可变全局表；smoke 断言 serial、单 job 和任务字段。 | 不得在 task 内复制函数体、AST、完整 `BodyIR` 或共享可变数组；不得把该切片写成 `ws` 完成。 |
| 只读快照 | 待实现 | `src/core/backend/lowering_plan.cheng`、`src/core/lang/typed_expr.cheng`、`src/core/ir/function_task.cheng` | `collect_globals + bind_imports + predeclare_top_level` 后冻结 `UirCoreSharedSnapshot`，worker 只能读。 | 新增 `src/tests/uir_fn_snapshot_mutation_negative_smoke.cheng` 后跑 `artifacts/backend_driver/cheng system-link-exec --root:/Users/lbcheng/cheng-lang --in:src/tests/uir_fn_snapshot_mutation_negative_smoke.cheng --emit:exe --target:arm64-apple-darwin --out:/tmp/uir_fn_snapshot_negative` | 任何 worker 侧共享状态写入都在编译期或 gate 中硬失败。 | 不得让 worker 直接改 AST SoA、全局 symbol table、type table 或 object layout。 |
| 串行 executor 作为 oracle | 当前已存在，需继续守住 | `src/core/ir/function_task_executor.cheng`、`src/core/backend/primary_object_plan.cheng` | `BACKEND_JOBS=1` 路径保持与旧 inline serial codegen 字节一致。 | `BACKEND_JOBS=1 artifacts/backend_driver/cheng system-link-exec --root:/Users/lbcheng/cheng-lang --in:src/tests/ordinary_zero_exit_fixture.cheng --emit:exe --target:arm64-apple-darwin --out:/tmp/cheng_fn_serial_oracle --report-out:/tmp/cheng_fn_serial_oracle.report.txt && rg -n 'primary_object_function_task_schedule|primary_object_function_task_job_count' /tmp/cheng_fn_serial_oracle.report.txt` | 单 job 仍是可复现 oracle；后续 `ws` 对照必须与它 SHA 一致。 | 不得把 oracle 作为用户侧成功路径接管 `ws` 失败。 |
| work-stealing executor | 待实现 | `src/core/ir/function_task_executor.cheng`、`src/std/sync.cheng`、必要时 `src/core/runtime/*provider*.cheng` | 纯 Cheng 任务执行器：按 `cost_hint/body_tok_span` 稳定 seed，本地 LIFO，跨 worker FIFO steal，结果按 `decl_ordinal` merge。 | 新增 `src/tests/function_task_ws_determinism_smoke.cheng` 后跑 `BACKEND_JOBS=1 ...` 与 `BACKEND_JOBS=4 ...` 两次 system-link-exec，并比较 report 与产物 SHA。 | `function_task_schedule=ws`、`job_count>1` 只在真实 executor 生效后出现；worker panic、queue 损坏、merge 冲突全部硬失败。 | 不得新增 C helper；不得用全局大锁把所有函数实际串行化；不得吞掉 worker 错误。 |
