...
权威来源：`docs/cheng-formal-spec.md`（版本 2026-02-07）。
...
- 导入仅允许归一化模块路径（`cheng/<pkg>/...`、`std/<name>`）。
- 内建字符串类型仅 `str`、`cstring`（`string` 非内建类型名）。
- 编译器 `stage1/mvp` 主链路会显式拒绝 `string` 类型名。
...
import cheng/libp2p/swarm
import cheng/libp2p/[crypto,transport]
```
