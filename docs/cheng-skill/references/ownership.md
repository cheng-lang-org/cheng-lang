...
- RC 默认原子实现（可用 `CHENG_MM_ATOMIC=0` 关闭以换取性能/单线程）；跨线程共享仍需 `share_mt/Arc[T]` 与 `Send/Sync` 边界检查。
...
- 写入全局/长生命周期容器：用 `share`/retain 防止提前释放；若容器 API 本身不 retain，则由 codegen 在 call-site 做 retain。
- alias 覆盖时先 retain 再 release，避免别名提前释放。
- `?` 解包赋值：Err 分支直接 return/panic，Ok 分支正常；未初始化 LHS 不 release。
- 若后端启用 moveHint，可在 last-use 场景跳过 retain，并在 cleanup 中跳过 moved-from 的 release（stage1 C 后端已移除；以当前后端实现为准）。
...
## 诊断
- `CHENG_OWNERSHIP_DIAGS=1`：编译期所有权诊断。
- `CHENG_MM_DIAG=1`：运行时 retain/release 日志与计数器。
