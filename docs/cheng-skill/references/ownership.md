...
- Cheng 没有 tracing GC；性能口径看 ORC retain/release 与 alloc/free/live，不看 tracing GC pause。
...
- no-pointer 生产口径下，用户源码模块默认禁指针；旧兼容名 `ABI=v2_noptr` 与 `STAGE1_NO_POINTERS_NON_C_ABI=1` 只保留在内部实现/兼容 gate 中，`@importc/@exportc` 也不会豁免 user-facing raw pointer 语法。
...
- `memRetainCount`/`memReleaseCount`/`memAllocCount`/`memFreeCount`/`memLiveCount` 与 `memDiagReset()` 可用于计数一致性检查和 ORC perf 闭环观察。

## FFI 与循环引用
- 跨 FFI 传递所有权需显式 `memRetain/memRelease`。
- 循环引用需显式打断（例如置空或拆分容器）。
- 默认公开表面是 no-pointer + borrow/`Send/Sync` 检查；但 FFI、handle 和环引用仍需显式管理，不能说成全域自动安全。
