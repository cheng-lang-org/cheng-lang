...
- CSG v2 口径：当前 `tools/cold_csg_v2_roundtrip_test.sh` 通过 1106/1106，`tools/cold_regression_test.sh` 通过 400/400；public `emit-cold-csg-v2` 输出 canonical `CHENG_CSG_V2` facts，internal `CHENGCSG` 只保留给显式 cold self-test。Linux AArch64 `write/getpid/get_nprocs` 已由 cold 生成真实 syscall provider object，并在 Docker `linux/arm64` 目标环境通过 `tools/linux_aarch64_provider_runtime_smoke.sh`，trace getpid/cpu marker 均成立；完整 Cheng `PrimaryObjectPlan` 管线仍未闭合；cold_parser 的 Lowering/Primary materializer 必须写 missing reason，不能空 plan 成功。
