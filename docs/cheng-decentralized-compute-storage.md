...
- **可控开关**：`CHENG_PKG_MODE=auto|local|p2p`、`CHENG_PKG_PEERS` 控制远程来源；lock 记录 `source_mode/source_addrs`。
...
# 环境变量（最小）
CHENG_PKG_MODE=p2p
CHENG_PKG_PEERS=/ip4/1.2.3.4/tcp/4001,/dns4/node.example/tcp/4001
CHENG_PKG_ALLOW=/dns4/node.example/tcp/4001
CHENG_PKG_DENY=/ip4/5.6.7.8/tcp/4001
```
...
### A1.21 `CHENG_PKG_*` 环境变量清单（建议）
- `CHENG_PKG_MODE`：`local|p2p|auto`
- `CHENG_PKG_PEERS`：逗号分隔 peer 列表
- `CHENG_PKG_ALLOW`：逗号分隔白名单
- `CHENG_PKG_DENY`：逗号分隔黑名单
- `CHENG_PKG_HTTP_FALLBACK`：`0|1`
- `CHENG_PKG_REQUIRE_SIGNATURE`：`0|1`
- `CHENG_PKG_REQUIRE_REGISTRY_MATCH`：`0|1`
...
`cheng-gui` 已提供 `ide/gui/services/p2p_bridge.cheng`，通过 `CHENG_IDE_P2P_LISTEN` 环境变量启用原生 libp2p 启动入口。
...
4) 编译（`chengc.sh` 会自动拉取并设置 `CHENG_PKG_ROOTS`）：
```bash
sh src/tooling/chengc.sh examples/your_app.cheng --manifest:doc/cheng-package-manifest.toml \
  --lock:build/cheng_pkg/cheng.lock.toml --registry:build/cheng_registry/registry.jsonl
```

说明：
- 包缓存默认 `chengcache/packages`，可通过 `--pkg-cache:<dir>` 或 `CHENG_PKG_CACHE` 调整。
- `CHENG_PKG_ROOT/CHENG_PKG_ROOTS` 用于解析 `cheng/<pkg>/...` 域名导入；未设置时默认 `~/.cheng-packages`；可指向包容器根，解析器会优先尝试 `cheng-<pkg>` 子目录。
...
```bash
CHENG_PKG_MODE=p2p CHENG_PKG_PEERS=/dns4/node.example/tcp/4001 \
  sh src/tooling/cheng_pkg_fetch.sh --lock:build/cheng_pkg/cheng.lock.toml --print-roots
...
## 统筹推进（与开发计划同步）
- 统筹执行计划见 `doc/cheng-dev-plan.md` 的 **去中心化计算与存储（统筹执行计划・TOML）**。
- 当前推进重点（P2 compute-mvp）：执行请求/回执协议、计量 SDK/宏、epoch 结算管线。
...
- 可选 IO backend：新增 `cheng/decentralized/io_backend.cheng`，支持 `cid://` 读写与 `CHENG_IO_MODE/ROOT/LEDGER/LISTEN/PEERS` 模式切换。
...
- IO backend 租约强制开关：`CHENG_IO_REQUIRE_LEASE=1` 且 `CHENG_IO_MODE=p2p` 时，`writeBytes/putFile/storeBytes` 会拒绝无租约写入。
...
# CHENG_IO_MODE=p2p CHENG_IO_ROOT=build/cheng_storage CHENG_IO_PEERS=/memory/1
# CHENG_IO_REQUIRE_LEASE=1
#
# # IO backend（租约写入）
...
# 如需 YAML 输出
./cheng_pkg resolve --manifest:doc/cheng-package-manifest.yaml --registry:build/cheng_registry/registry.jsonl \
...
manifest 示例（`doc/cheng-package-manifest.toml`）：
```toml
package_id = "pkg://cheng/app-demo"
...
manifest 示例（`doc/cheng-package-manifest.yaml`）：
```yaml
package_id: "pkg://cheng/app-demo"
dependencies:
  - package_id: "pkg://cheng/fs"
