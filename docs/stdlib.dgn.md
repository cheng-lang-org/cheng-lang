...
####swap
Swap two values. This does not use and `=sink` or `=copy` or `=dup` hooks! It always swaps the values' bits. This is in line with the design of Cheng's move semantics.
...
Borrows a C string from a Cheng string. This is unsafe because the Cheng string may be moved or deallocated while the C string is still in use.
...
Converts a Cheng string to a C string.

####prepareMutation

Prepares a string for mutation. String literals are "copy on write", so you need to call `prepareMutation` before modifying strings via `&`.
...
Concatenates two strings. For many pieces, prefer building a `seq[str]` and using `std/strutils.join`.
...
`ord` 已移除；获取序数值请使用显式类型转换，例如 `int32(x)` 或 `uint64(x)`。
字符字面量（如 `'\n'`、`'0'`）直接表示对应的 ASCII 数值。
...
####getCurrentDir
Returns the current working directory i.e. where the built binary is run. The path returned by this fn is determined at run time.

####normalizeExe
Normalize executable name. On Windows this fn will check if an `.exe` extension needs to be added. On other platforms it does nothing.
