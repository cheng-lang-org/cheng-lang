...
| CSG v2 facts 往返 | 99% | **1106/1106 PASS（0 FAIL）**；真实 backend driver canonical facts → cold reader → provider archive → `--csg-in --provider-archive --emit:exe` 直接入口闭合 |
| PrimaryObjectPlan → facts | 85% | `primary_object_plan.cheng`（1.1MB）冷编译→1.60MB Mach-O .o（0 errors）；三 intrinsic 已启用 |
| cold 回归 / --emit:obj | 98% | **400/400 回归 PASS（100%，0 FAIL）**；模块级 compile smoke 锁 `emit=obj/direct_macho/system_link=0/linkerless_image=1` report 合同 |
| cold linkerless exe | 82% | canonical exe 三路径全链闭合 |
| provider archive | 97% | ELF + Mach-O provider archive pack 通过；AArch64 ELF `--csg-in --provider-archive --emit:exe` 已闭合；Darwin Mach-O runtime 纯常量 root provider archive link/run 已锁；Linux AArch64 `write/getpid/get_nprocs` 已由 cold 生成真实 syscall provider object，并在 Docker `linux/arm64` 目标环境通过 `tools/linux_aarch64_provider_runtime_smoke.sh` trace getpid/cpu marker；错 target、错 machine、重复导出、歧义导出、primary/provider 定义冲突已 hard-fail 锁定 |
...
| 包管理器 | 0% | 设计文档 `docs/cheng-package-manager.md` 已完成；无代码实现 |
| Debugger（DWARF） | 0% | 设计文档 `docs/cheng-debugger.md` 已完成；无代码实现 |
| LSP 语言服务器 | 0% | 设计文档 `docs/cheng-lsp.md` 已完成；无代码实现 |
...
- **CSG v2 facts 往返**：`tools/cold_csg_v2_roundtrip_test.sh` 1106/1106 PASS
...
- **provider archive / runtime target smoke**：`provider-archive-pack` 生成 `.chenga`；AArch64 ELF canonical facts 已通过 `--csg-in --provider-archive --emit:exe` 直接入口，report 锁 `provider_archive_member_count=2`、`provider_export_count=2`、`provider_resolved_symbol_count=2`、`system_link=0`、`linkerless_image=1`；Darwin Mach-O 已锁 runtime 纯常量 root `cheng_native_af_inet_bridge` 的 provider archive link/run；Linux AArch64 `write/getpid/get_nprocs` 已由 cold 生成真实 syscall provider object，`runtime_provider_autolink_trace` 与 `runtime_provider_autolink_cpu_cores` 锁 `provider_archive_member_count=2`、`provider_export_count=10`、`provider_resolved_symbol_count>0`、`unresolved_symbol_count=0`；`tools/linux_aarch64_provider_runtime_smoke.sh` 已在 Docker `linux/arm64` 目标环境运行通过，trace getpid stderr marker 与 cpu cores 非零 marker 均成立；缺 native Linux AArch64、`qemu-aarch64`/`qemu-aarch64-static` 或显式 runner 时仍直接失败；错 target、错 machine、重复导出、歧义导出、primary/provider 定义冲突已锁
- **E-Graph 合同边界**：CSE 已移除；当前保留 DSE、24+ rewrite rules（整数/位运算恒等式 + 强度缩减，含 EQ(x,x)→CONST 1 identity、double-NEG/NOT 消除）带 intra-block 安全证明；跨块恒等式只允许 single-writer + dominator 证明后 rewrite，未证明直接不改写。`normalization_coverage=I32_I64_bitwise_integer_only`，浮点不参与 canonical hash 操作数重排。LICM CONST hoisting 已实现但不在 codegen 主线中。没有 UIR E-Graph。
...
| 函数级并行 | C 层 codegen 并行已激活；Cheng 层 `LoweringBuildPrimaryObjectIr` 已接入 `FunctionTaskExecuteAuto` active lowering 主链，`BACKEND_JOBS=1/4` 产物确定性已验证 | 继续补 benchmark 证明加速比，扩大失败传播门禁 |

### 本次会话（2026-05-16）：Linux AArch64 provider 真实目标运行闭合，400/400 回归 0 FAIL，CSG v2 1106/1106

**回归**：**400/400**（**0 FAIL**）。**CSG v2**：**1106/1106**（0 FAIL）。**Provider archive**：Linux AArch64 `write/getpid/get_nprocs` 已从链接报告推进到真实目标运行证明；`runtime_provider_autolink_trace` 和 `runtime_provider_autolink_cpu_cores` 均通过 `--link-providers`，报告锁 `provider_object_count=2`、`provider_archive_member_count=2`、`provider_export_count=10`、`provider_resolved_symbol_count>0`、`unresolved_symbol_count=0`、`system_link=0`。`tools/linux_aarch64_provider_runtime_smoke.sh` 已在 Docker `linux/arm64` 环境运行通过，`uname` 为 `Linux/aarch64`，trace getpid stderr marker 与 cpu cores 非零 marker 均成立。

### 本次会话（2026-05-15/16）：C seed 70→100%，184/184 回归 0 FAIL，CSG v2 859/859

**回归**：92→**184/184**（**0 FAIL**）。**CSG v2**：→**859/859**（0 FAIL）。**Provider archive**：73→93%（Mach-O pack + Darwin runtime 纯常量 root link/run + 外部系统依赖 hard-fail 分类 + AArch64 canonical direct provider archive entry + provider archive 负向合同）。全部可修复测试通过。dispatch_min、pure_backend_driver、import_deep、import_unresolved、compiler_runtime_smoke、ownership_proof 全部修复。SIGBUS crash（longjmp to freed frame）已修复。C bridge unresolved patch 噪声已消除。
...
**184/184 回归全通（0 FAIL）**：所有可修复测试通过。dispatch_min、pure_backend_driver、import_deep、import_unresolved、compiler_runtime_smoke、ownership_proof 全部修复。C bridge 未解析补丁噪声消除。
...
- 旧 async 线程入口迁移到 `std/thread.SpawnPtr`（3 文件）

**追加（2026-05-16）：SIGBUS 修复 + pure_backend_driver + 184/184 回归**
...
- **184/184 回归**：pure_backend_driver PASS + import_deep hard-fail 加入回归集，Linux runtime provider 不完整路径改为结构化 hard-fail，模块级 cold compile smoke 严格锁 report 合同
...
**本地实跑**：
- 当时本地实跑通过；当前基线见上方 400/400 与 1106/1106
...
- **CSG v2 fixtures**：当前总门禁 859，含 canonical writer/read/link/run smoke、显式 `--link-object` smoke、provider archive 负向合同、Darwin runtime 纯常量 root provider archive link/run、provider 外部系统依赖 hard-fail 与 internal fixed-point。
...
- 旧 runtime 占位实现已删除；原子 provider smoke 只证明 atomic 外部符号解析；Linux runtime roots 已通过 root-selective 自动归档链接报告门禁，含 `write/getpid/get_nprocs` syscall-backed roots；Docker `linux/arm64` 真实目标运行门禁已覆盖 trace getpid/cpu marker。
...
- **历史回归矩阵**：28/28 冷编译器回归 PASS，13/13 bootstrap slice PASS，3/3 kernel PASS；当前基线见上方 400/400。
...
- **并行状态审计与 roadmap 更新**：审计了 C 层和 Cheng 层两套并行框架的实际状态。
  ...
  - **Cheng 层 lowering 并行**（`src/core/backend/lowering_plan.cheng`）：`LoweringBuildPrimaryObjectIr` 已通过 `FunctionTaskExecuteAuto` 调度 `FunctionTaskExecuteBodyIr`，`BACKEND_JOBS=1/4` object 产物确定性已验证；`src/core/ir/function_task_executor.cheng` 的独立 contract smoke 仍保留为 executor 合同。
  ...
  - **最小接入路径已落地**：`FunctionTaskExecuteBodyIr` 调用 `LoweringBuildOneFunction`，`LoweringBuildPrimaryObjectIr` 已切换到 `FunctionTaskExecuteAuto` 调度。
...
- **lowering_plan.cheng 并行化尝试**（2026-05-08 提交 `9c40953d`、`3816cae3`、`bf077095`）：
  - 曾用旧 async 入口 + `atomic.FetchAddI32` 尝试 fan-out 并行 for 循环，替代 `LoweringBuildPrimaryObjectIr` 中的串行 `for`
...
- **保留边界**：
  ...
  - `lowering_plan.cheng` 已接入 active lowering 主链；剩余缺口是扩大 benchmark 和失败传播门禁
...
**当前状态**：provider archive 已成为 cold linkerless 已验证入口之一。`provider-archive-pack` 能把多个 provider `.o` 和多个 export 写入 `.chenga`；`system-link-exec --link-object --provider-archive` 与 `system-link-exec --csg-in --provider-archive` 都能把 primary undefined symbols 解析到 archive members，并生成 ELF/Mach-O executable。Darwin 已验证 runtime 纯常量 root `cheng_native_af_inet_bridge` 的 provider archive link/run。Linux AArch64 `write/getpid/get_nprocs` 已由 cold 生成真实 syscall provider object，`--link-providers` 已通过 trace getpid/cpu cores 两个 syscall-backed root 的正向链接和 Docker `linux/arm64` 运行门禁。原子 provider archive smoke 只证明 `atomicCasRaw/atomicStoreRaw/atomicLoadRaw` 外部符号解析，不能外推成全部 runtime roots 完成。

**阻断原因**：纯常量 runtime roots 已锁链接报告/部分运行 marker，Linux AArch64 `write/getpid/get_nprocs` 已正向闭合；更多 libc/系统调用依赖 root 仍缺逐项 syscall/provider object 实现和可运行目标门禁。
...
有两层并行：C 层（codegen 阶段，已激活）和 Cheng 层（lowering 阶段，已接入 active 主链）。
...
**Cheng 层 lowering 并行（已接入 active 主链）**：
...
- `FunctionTaskExecuteBodyIr` 的实际 lowering 实现已接入 `LoweringBuildPrimaryObjectIr`
- **`LoweringBuildPrimaryObjectIr` 在 `lowering_plan.cheng` 中通过 `FunctionTaskExecuteAuto(functionCount, jobCount)` 调度；`BACKEND_JOBS=1/4` object 产物确定性已验证**
- `backend_driver_dispatch_min.cheng` 保留 `function_task_schedule=ws/serial` 动态报告，但该报告只反映 `cold_jobs_from_env()` 的值，与 lowering 并行无关
- active 客户端：`LoweringBuildPrimaryObjectIr`。`function_task_executor_contract_smoke.cheng` 仍保留为独立 executor 合同测试。
- **2026-05-08 尝试**：用旧 async 入口 + `atomic.FetchAddI32` 实现 fan-out 并行 for 循环。代码语法/语义通过，但 `primary_object_plan` codegen 不支持 `let`/`if`/`while`/atomic/spawn 等语句模式，产出 `unsupported_body_kinds`，最终 C seed 链接失败。根因：Cheng 规范 §716 禁止循环模块导入，不能走 `function_task_executor ↔ lowering_plan` 回调；codegen 不支持函数体内的控制流/并发原语。

**当前边界**：
1. `LoweringBuildPrimaryObjectIr` 已接入 `FunctionTaskExecuteAuto`
2. `FunctionTaskExecuteBodyIr` 已调用 `LoweringBuildOneFunction` 并把结果写入确定性结果数组
3. `BACKEND_JOBS=1/4` 已验证 object 产物确定性
4. 仍需补失败传播和 benchmark 门禁，防止 worker 失败被错误聚合成成功

**剩余完成条件**：
1. worker 任一失败必须 hard-fail，不能串行接管
2. report 明确写 `function_task_schedule=ws`、`job_count>1`
3. benchmark 证明加速比（>=2 函数时 wall clock 下降）
4. smoke 必须检查 marker，防止入口被折成直接 `return 0` 假绿

**预计工作量**：~2-4 天。核心接入已完成，剩余是性能证明和失败路径门禁。
...
| 开发者工具链 | Debugger (DWARF) + LSP 语言服务器 | 设计文档已就位；无代码实现 |
...
| `bootstrap/cheng_cold.c` | 只允许修 blocker 或删旧路径，不新增生产能力 | `cc -std=c11 -Wall -Wextra -pedantic -fsyntax-only bootstrap/cheng_cold.c` | cold bootstrap 只作外根与 blocker 修复，不恢复 C seed 默认路径 |
...
| `FunctionTaskExecuteWorkStealing` 框架 | **已完成** | `src/core/ir/function_task_executor.cheng:182-229`：`thread.SpawnPtr` + `atomic.CasI32` + Chase-Lev pop/steal |
...
| `LoweringBuildPrimaryObjectIr` 接入 executor | **已完成** | `src/core/backend/lowering_plan.cheng` 调用 `FunctionTaskExecuteAuto(functionCount, jobCount)`，`BACKEND_JOBS=1/4` object 产物确定性已验证 |
...
fn LoweringBuildPrimaryObjectIr(...): PrimaryObjectIr =
    loweringParallelPlan = plan
    loweringParallelSourceTexts = sourceTexts
    loweringParallelResults = []
    for i in 0..<functionCount:
        var empty: PrimaryObjectIrFunction
        add(loweringParallelResults, empty)
    FunctionTaskExecuteAuto(functionCount, jobCount)
    for i in 0..<functionCount:
        add(ir.functions, loweringParallelResults[i])
    loweringParallelResults = []
    return ir
```
...
- cold source-direct runtime smoke 已运行通过；Darwin runtime marker object、Mach-O provider archive pack、纯常量 root provider archive link/run、缺失 archive hard-fail 已锁，Linux AArch64 `write/getpid/get_nprocs` 已由 cold syscall provider object 正向链接并在 Docker `linux/arm64` 运行，更多 libc/系统符号依赖的目标运行 marker 继续扩展 ⚠️
- cross-target smoke 证明 Mach-O/ELF/COFF 产物合法 ✅

## 阶段 8：开发者工具链 (Debugger + LSP)

目标：为 Cheng 提供源级调试和 IDE 支持，提升开发体验。

### 8.1 Debugger（DWARF）

设计文档：`docs/cheng-debugger.md`

**方案**：冷编译器 codegen 阶段发射 DWARF 调试信息，复用 lldb/gdb 作为调试前端。不做自定义调试器。

**三相实现**：

| 相 | 功能 | DWARF 支持 | 工作量 |
|---|---|---|---|
| 1 (MVP) | 函数级断点 + 行号回退 | `.debug_line`, `.debug_info` (CU + subprogram), `.debug_abbrev`, `.debug_str` | 3-5 天 |
| 2 | 局部变量查看 | `DW_TAG_variable`, `DW_TAG_formal_parameter`, `.debug_loc` | 1-2 周 |
| 3 | 类型信息 + 表达式求值 | 完整类型 DAG (base types, struct, enum, array, pointer, union) | 2-4 周 |

**冷编译器改动**：
- `bootstrap/cheng_cold.c`：新增 `cold_dwarf_emit_line_table` / `cold_dwarf_emit_abbrev` / `cold_dwarf_emit_info` / `cold_dwarf_emit_str` 四个发射函数。
- `bootstrap/macho_direct.h`：Mach-O 写入 `__DWARF,__debug_info` 等 section。
- `bootstrap/cold_parser.c`：暴露每函数的源文件行号范围。
- ELF64/COFF writer：对应添加 `.debug_*` section。

### 8.2 LSP 语言服务器

设计文档：`docs/cheng-lsp.md`

**方案**：独立二进制 `cheng-lsp`，链接 `cold_parser.c`，通过 JSON-RPC 2.0 over stdio 与编辑器交互。

**三相实现**：

| 相 | 功能 | 冷解析器能力 | 工作量 |
|---|---|---|---|
| 1 (MVP) | 诊断错误 + 语义高亮 | 已有的 tokenization、error reporting | 3-5 天 |
| 2 | 跳转定义 + 查找引用 + Hover | 已有的符号表、类型决议 | 3-4 天 |
| 3 | 自动补全 + 重命名 + Code Action | 需新增作用域追踪 | 5-7 天 |

**新增文件**：
- `src/tooling/cheng_lsp_main.c`：LSP 服务器主入口。
- `src/tooling/lsp_protocol.h`：LSP 协议类型和 JSON 序列化。

### 8.3 依赖关系

- Phase 1 of both can proceed in parallel (no shared dependencies).
- DWARF Phase 2 depends on Phase 1 being complete.
- LSP Phase 2 depends on Phase 1 being complete.
- DWARF Phase 3 and LSP Phase 3 are independent of each other.
...
7. ✅ **回归测试**：`tools/cold_regression_test.sh` 400/400 PASS，`tools/cold_csg_v2_roundtrip_test.sh` 1106/1106 PASS。
8. ✅ **cold source-direct runtime smoke 通过**：`atomic_i32_runtime_smoke`、`thread_atomic_orc_runtime_smoke`、`compiler_runtime_smoke`、source-direct `while` 均通过；Linux runtime provider roots 已锁链接报告，`write/getpid/get_nprocs` 已在 Docker `linux/arm64` 目标环境运行通过，更多 roots 继续扩展。
9. 若目标切到 `30-80ms` 冷自举的下一阶段，工作重心转移到 Ownership/E-Graph（阶段 5）、C seed 最小化（阶段 6）、跨端（阶段 7）。
10. 🔲 **开发者工具链**：设计文档已就位（`docs/cheng-debugger.md`、`docs/cheng-lsp.md`），无代码实现。编译器收敛后可作为下一阶段方向。
