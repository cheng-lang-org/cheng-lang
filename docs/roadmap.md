...
| cold --csg-in --emit:obj | 80% | 最小 fixture、cold subset、provider archive smoke 稳定；77/77 回归 + 716/716 CSG v2 PASS；runtime provider 纯常量 root 集已接入 root-selective archive 链接报告门禁 |
| cold linkerless exe | 77% | provider-free 可行；A/B witness 通过；多 provider archive ELF 链路、`--csg-in --provider-archive`、10 个 runtime provider 纯常量 roots 自动归档链接报告已由门禁覆盖 |
| provider archive | 72% | `provider-archive-pack` + `--link-object/--csg-in --provider-archive` 已覆盖多 ELF member/export、缺 export hard-fail；Mach-O archive 入口已明确硬失败；10 个 Linux runtime 纯常量 roots 已走 primary undefined symbol 自动选 root 并锁链接报告 |
...
| gap | 状态 | 下一步 |
|---|---|---|
| provider archive 生成与链接 | 多 provider ELF member/export 已由 cold linkerless 链路闭合；Darwin runtime marker object 已锁；Mach-O archive pack/link 三入口已明确硬失败；10 个 Linux runtime 纯常量 roots 已按 primary undefined symbols 自动选择并锁链接报告 | 扩展非纯常量 runtime provider root 集，并补齐错 target、错 machine、重复导出、primary/provider 定义冲突硬失败门禁 |
...
**本地实跑**：
- 77/77 冷回归 PASS，716/716 CSG v2 roundtrip PASS ✅
- `ordinary_zero_exit_fixture` provider-backed 编译运行 exit 0 ✅
- `atomic_i32_runtime_smoke` / `compiler_runtime_smoke` 全部通过 ✅

**本轮有效推进**：
- `tools/cold_regression_test.sh` 77/77 PASS，`tools/cold_csg_v2_roundtrip_test.sh` 716/716 PASS。
...
- **+323 CSG v2 fixtures，回归矩阵继续扩展**：CSG v2 数量跃升至 716，冷回归当前拓展至 77（含 float/recursion/string WIP fixtures，commit 519d8a70、943be30f）。
...
- 旧 runtime 占位实现已删除；原子 provider smoke 只证明 atomic 外部符号解析；真实 Linux runtime provider 已完成 10 个纯常量 roots 的 root-selective 自动归档链接报告门禁。
...
- **当前边界**：archive 已支持多 ELF member/export，目标限定在当前 cold ELF reloc linker 支持的 AArch64/RISC-V；runtime provider 纯常量 roots 已锁链接报告，非纯常量 roots 和目标运行 marker 仍需逐个纳入；Darwin Mach-O provider archive reader/linker 不支持并已硬失败锁定。
...
- **历史回归矩阵**：28/28 冷编译器回归 PASS，13/13 bootstrap slice PASS，3/3 kernel PASS；当前基线见上方 77/77。
...
**阻断原因**：纯常量 runtime roots 已锁链接报告，但非纯常量 runtime root 集和尚未执行目标 ELF 的运行 marker 尚未展开；还需要 target/machine 校验、重复导出 hard-fail、primary/provider 定义冲突 hard-fail。
...
- cold source-direct runtime smoke 已运行通过；Darwin runtime marker object 与 Mach-O provider archive 硬失败已锁，Linux runtime provider 纯常量 roots 已自动归档链接报告，非纯常量 root 与目标运行 marker 继续扩展 ⚠️
...
7. ✅ **回归测试**：`tools/cold_regression_test.sh` 77/77 PASS，`tools/cold_csg_v2_roundtrip_test.sh` 716/716 PASS。
8. ✅ **cold source-direct runtime smoke 通过**：`atomic_i32_runtime_smoke`、`thread_atomic_orc_runtime_smoke`、`compiler_runtime_smoke`、source-direct `while` 均通过；Linux runtime provider 纯常量 roots 已自动归档链接报告，非纯常量 root 与目标运行 marker 继续扩展。
