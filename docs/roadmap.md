...
| C seed 替代 | 70% | 8 blockers fixed；**38/39 源文件可冷编译**（`--emit:obj`），**38/42 manifest 文件可冷编译**；仅剩 `parser.cheng`（8054 行）——阻断于 `unsupported for range end expr`：`lookup->buckets[bucketIndex].items.len` 涉及链式字段访问+数组索引的 range end 表达式；已修复：`gate_main`(1.9MB, last-chance import alias constant/variant 回退)、`primary_object_plan`(1.1MB, SLOT_OBJECT↔SLOT_OBJECT kind + size skip)、rawmem intrinsic 桩、SLOT_OBJECT_REF 宽限 SEQ_OPAQUE/SEQ_OPAQUE_REF；函数级泛型/完整单态化未闭合，cheng_seed.c 仍在链中 |
