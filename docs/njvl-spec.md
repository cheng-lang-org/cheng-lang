...
NJVL is an intermediate representation for Cheng.
...
- Computing the validity of Cheng's `requires` and `ensures` contracts.
...
The `cfvar` tag declares a new control flow variable. It is always of type `bool` and initialized to `false`.
...
For readability we use Cheng syntax here, but the actual IR uses the NJVL form.
