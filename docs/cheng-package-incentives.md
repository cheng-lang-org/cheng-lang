...
- 允许配置 `CHENG_PKG_MODE=local|p2p|auto` 与 `CHENG_PKG_PEERS`，并支持白名单/黑名单策略。
...
```bash
python src/tooling/cheng_pkg_rewards.py \
  --input doc/cheng-package-incentives-example.json \
...
./cheng_pkg_rewards --input doc/cheng-package-incentives-example.json --out build/cheng-package-incentives-payouts.json --pretty
```
