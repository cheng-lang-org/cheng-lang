...
*Cheng* is based on Cheng version 2.0, so all of its lexing rules and syntax rules apply here too. In fact Cheng's manual starting from its [lexical analysis](https://cheng-lang.org/docs/manual.html#lexical-analysis) and up to and including its [grammar](https://cheng-lang.org/docs/manual.html#syntax-grammar) section are valid.

There are two important exceptions:

- **Cheng is case sensitive** like most other modern programming languages. The reason for this is implementation simplicity. This might also be changed in the future.
- **Single-argument calls may omit parentheses only at the outermost call position**: write `f x` for the outermost call; use `f(x)` for nested single-argument calls. Parentheses are still used for zero-argument calls, multi-argument calls, or named arguments; type conversions use `Type(x)`.
...
A module's name can be used to disambiguate names. For example if both the `system` and a `foreign` module contain a `delay` fn, `system.delay` can be used to clarify that it is the `system` module's `delay` fn.
...
Cheng offers a complex evaluation engine for arbitrary compile-time computations. The restrictions for this mechanism are currently undocumented.
...
This condition holds:
  ...
  ```cheng
  int32(false) == 0 && int32(true) == 1
  ```

The operators `!`, `&&`, `||`, `^`, `<`, `<=`, `>`, `>=`, `!=`, `==` are defined
for the bool type. The `&&` and `||` operators perform short-cut
evaluation. Example:

  ```cheng
  while p != nil && p.name != "xyz":
...
Character literals can be used in numeric expressions and represent their ASCII value
(for example, `'0'` in arithmetic).
...
enums as an index type for arrays. The fns `inc`, `dec`, `succ`
...
String concatenation uses the `+` operator. Concatenating many strings with `+` creates intermediate copies; prefer collecting into a `seq[string]` and using `strutils.join` when building large strings.
...
A `$` fn is defined for cstrings that returns a string. Thus, to get a cheng
...
sequence is to call the built-in `newSeq` fn.
...
The lower bound of an array or sequence may be received by the built-in fn
...
One can append elements to a sequence with the `add()` fn,
and remove (and get) the last element of a sequence with the `pop()` fn.
...
`array` and `seq` values can be passed directly to `openArray` parameters
without any explicit operation:

  ```cheng
  fn testOpenArray(x: openArray[int]) = echo repr(x)
...
`varargs` is a pragma that can be applied to `importc` fns to indicate
the fn takes a variable number of arguments (`printf` being the typical
example). It can also be applied to a template. Inside a template the args
can then be unpacked like so:

```cheng
template joinText(): string {.varargs.} =
  var res = ""
  for a in unpack():
    res = res + a
  res
...
Cheng's `echo` also uses `varargs`:
...
with an object construction expression or the fn `default`; a `ref object` created
with an object construction expression or the fn `new`.
...
block: # created with an object construction expression
  let x = Foo()
  assert x.a == 2 && x.b == 3.0

  let y = Bar()
  assert y.a == 2 && y.b == 3.0
...
block: # created with an object construction expression
  ...
  assert y[0].a == 2 && y[0].b == 3.0

  let z = default(tuple[x: Foo])
  assert z.x.a == 2 && z.x.b == 3.0

block: # created with the fn `new`
  let y = new Bar
  assert y.a == 2 && y.b == 3.0
```
...
Cheng distinguishes between `traced`:idx: and `untraced`:idx: references.
...
Cheng also supports a C-style surface syntax: `T*` is accepted in type
positions as a synonym for `ptr T` (repeatable for multiple indirection),
`*p` dereferences a pointer, `&x` takes an address, and `p->field` is
syntactic sugar for `p[].field`. The `ref` model stays unchanged.

An empty subscript `[]` notation can be used to de-refer a reference,
the `&` operator returns the address of an item. If `x` has the type `T`
then `&x` has the type `ptr T`.
Thus, taking addresses is an *unsafe* feature.
...
To deal with untraced memory, the fns `alloc`, `dealloc` and
...
Cheng supports `not nil` tracking, which allows the compiler to track when a reference is guaranteed to be non-nil. This is done by appending `not nil` to a reference type:
...
A procedural type is internally a pointer to a fn. `nil` is
...
fn influences the type compatibility: procedural types are only
compatible if they have the same calling convention.

Cheng supports these `calling conventions`:idx:\:

`chengcall`:idx:
:   is the default convention used for a Cheng **fn**. It is the
    same as `fastcall`, but only for C compilers that support `fastcall`.

`closure`:idx:
:   is the default calling convention for a **procedural type** that lacks
    any pragma annotations. It indicates that the fn has a hidden
    implicit parameter (an *environment*). Proc vars that have the calling
    convention `closure` take up two machine words: One for the fn pointer
...
:   This is the stdcall convention as specified by Microsoft. The generated C
    fn is declared with the `__stdcall` keyword.

`cdecl`:idx:
:   The cdecl convention means that a fn shall use the same convention
    as the C compiler. Under Windows the generated C fn is declared with
    the `__cdecl` keyword.

`safecall`:idx:
:   This is the safecall convention as specified by Microsoft. The generated C
    fn is declared with the `__safecall` keyword. The word *safe*
    refers to the fact that all hardware registers shall be pushed to the
    hardware stack.

`inline`:idx:
:   The inline convention means the caller should not call the fn,
    but inline its code directly. Note that Cheng does not inline, but leaves
    this to the C compiler; it generates `__inline` fns. This is
    only a hint for the compiler: it may completely ignore it, and
    it may inline fns that are not marked as `inline`.

`noinline`:idx:
:   The backend compiler may inline fns that are not marked as `inline`.
...
:   The generated C code will not have any explicit calling convention and thus
    ...
    Cheng's default calling convention for fns is `fastcall` to
    improve speed.

Most calling conventions exist only for the Windows 32-bit platform.

The default calling convention is `chengcall`.
...
Cheng uses structural type equivalence for most types. Only for objects,
...
algorithm returns true:
  ...
  ```cheng
  fn isImplicitlyConvertible(a, b: PType): bool =
    if isSubtype(a, b):
      return true
    if isIntLiteral(a):
    ...
    of seq:
      result = b == openArray && typeEquals(a.baseType, b.baseType)
    of array:
      result = b == openArray && typeEquals(a.baseType, b.baseType)
      if a.baseType == char && a.indexType.rangeA == 0:
        result = b == cstring
    of cstring, ptr:
      result = b == pointer
    of string:
      result = b == cstring
    of fn:
      result = typeEquals(a, b) || compatibleParametersAndEffects(a, b)
  ```
...
Implicit conversions are also performed for Cheng's `range` type
constructor.
...
exists if `a0 >= low(F) && b0 <= high(F)` and both `T` and `F`
...
algorithm returns true:
  ...
  ```cheng
  fn isIntegralType(t: PType): bool =
    result = isOrdinal(t) || t.kind in {float, float32, float64}

  fn isExplicitlyConvertible(a, b: PType): bool =
    result = false
    if isImplicitlyConvertible(a, b): return true
    if typeEquals(a, b): return true
    if a == distinct && typeEquals(a.baseType, b): return true
    if b == distinct && typeEquals(b.baseType, a): return true
    if isIntegralType(a) && isIntegralType(b): return true
...
6. Conversion match: `a` is convertible to `f` under implicit conversion
   rules.
...
### Some Examples
  ...
  ```cheng
  fn takesInt(x: int) = echo "int"
  fn takesInt[T](x: T) = echo "T"
  fn takesInt(x: int16) = echo "int16"
...
into account:
  ...
  fn pp(obj: A, obj2: B) = echo "A B"
  fn pp(obj: B, obj2: A) = echo "B A"
...
matches) is preferred:
  ...
  ```cheng
  fn gen[T](x: ref ref T) = echo "ref ref T"
  fn gen[T](x: ref T) = echo "ref T"
  fn gen[T](x: T) = echo "T"
...
- In a function call `q(..., p, ...)` when the corresponding formal parameter
  of `q` is a `fn` type. If `q` itself is overloaded then the cartesian product
  of every interpretation of `q` and `p` must be considered.
- In an object constructor `Obj(..., field: p, ...)` when `field` is a `fn`
  type. Analogous rules exist for array/set/tuple constructors.
- In a declaration like `x: T = p` when `T` is a `fn` type.
...
must be of type `void`. In Cheng, expression statements can ignore their
return values directly. `(;)` does not introduce a new scope.


### Expression statements

An expression can appear as a statement to evaluate its side-effects and
ignore its return value. Cheng does not require a special keyword for this.
...
Example:
  ...
  p(3, 4) # ignore the return value of `p`
  ```

A no-op branch can use a literal expression:

  ```cheng
  fn classify(s: string) =
    case s[0]
...
If a fn is annotated with the `noinit` pragma, this refers to its implicit
`result` variable:

  ```cheng
  fn returnUndefinedValue: int {.noinit.} = 0
  ```
...
The special identifier `_` can be used to ignore some parts of the tuple:
  ...
  ```cheng
  fn returnsTuple(): (int, int, int) = (4, 2, 3)
...
Tuple unpacking can also be nested:
  ...
  ```cheng
  fn returnsNestedTuple(): (int, (int, int), int, int) = (4, (5, 7), 2, 3)
...
To suppress the static error an `else: 0` branch can be used.
...
The `return` statement ends the execution of the current fn.
It is only allowed in fns, funcs, methods and templates. If used in a template
...
the fn has a return type. The `result`:idx: variable is always the return
value of the fn. It is automatically declared by the compiler.
...
```cheng
fn foo(c: Context; n: Node) = ...
fn bar(c: Context; n: Node, counter: int) = ...
fn baz(c: Context; n: Node) = ...
```
...
effects. When multiple statements are given for a branch, Cheng will use
...
Syntactically a *type conversion* is like a fn call, but a
type name replaces the fn name. A type conversion is always
safe in the sense that a failure to convert a type to another
results in an exception (if it cannot be determined statically).

Ordinary fns are often preferred over type conversions in Cheng: For instance,
...
Type conversion can also be used to disambiguate overloaded routines:
  ...
  ```cheng
  fn p(x: int) = echo "int"
  fn p(x: string) = echo "string"

  let procVar = (fn(x: string))(p)
  procVar("a")
  ```
...
language when algorithms are ported from C to Cheng.
...
Cheng uses `TypeExpr(expr)` for explicit casts when `TypeExpr` is a type
expression and the call has a single argument. 仅支持 `T(x)`；C 风格
`(T)expr`/`(T)(expr)` 与 `cast[T](x)` 已移除。
...
that is a type class (which is non-concrete) would be invalid:
  ...
  var x = Foo(1) # Error: cannot cast to a non concrete type: 'Foo'
  ```
...
The `&` operator returns the address of an l-value. If the type of the
location is `T`, the `&` operator result is of the type `ptr T`. An
...
every time the function is called.
  ...
  fn foo(a: int; b: int = 47): int
  ```

If the fn returns a value, the fn body can access an implicitly declared
variable named `result`:idx: that represents the return value. Procs can be
overloaded. The overloading resolution algorithm determines which fn is the
best match for the arguments. Example:

  ```cheng
  fn toLower(c: char): char = # toLower for characters
    if c in {'A'..'Z'}:
      result = char(int(c) + (int('a') - int('A')))
    else:
      result = c

  fn toLower(s: string): string = # toLower for strings
    result = newString(len(s))
    for i in 0..len(s) - 1:
      result[i] = toLower(s[i]) # calls toLower for characters; no recursion!
  ```

Calling a fn can be done in many ways:

  ```cheng
  fn callme(x, y: int, s: string = "", c: char, b: bool = false) = ...
...
A fn may call itself recursively.
...
Any operator can be called like an ordinary fn with the \`opr\`
...
Typed bindings without an initializer are implicitly default-initialized:

  ```cheng
  fn test =
    var s: seq[string] # defaults to empty seq
    s.add "abc" # valid!
  ```

Explicit empty-seq initializers in a typed binding are not allowed; omit the initializer instead.
...
The type of a parameter may be prefixed with the `var` keyword:
  ...
  ```cheng
  fn divmod(a, b: int; res, remainder: var int) =
    res = a div b
    remainder = a mod b
...
Var parameters can be modified by the fn and the changes are
...
above example is equivalent to:
  ...
  ```cheng
  fn divmod(a, b: int; res, remainder: ptr int) =
    res[] = a div b
    remainder[] = a mod b
...
return values. This can be done in a cleaner way by returning a tuple:
  ...
  ```cheng
  fn divmod(a, b: int): tuple[res, remainder: int] =
    (a div b, a mod b)
...
A fn or iterator may return a `var` type which means that the
returned value is an l-value and can be modified by the caller:

  ```cheng
  var g = 0

  fn writeAccessToG(): var int =
    result = g
...
then it has to be derived from the routine's first parameter:
  ...
  ```cheng
  fn forward[T](x: var T): var T =
    result = x # ok, derived from the first parameter.

  fn p(param: var int): var int =
    var x: int
...
Later versions of Cheng can be more precise about the borrowing rule with
a syntax like:

  ```cheng
  fn foo(other: Y; container: var X): var T from container
  ```
...
An `out` parameter is like a `var` parameter but it must be written to before it can be used:
  ...
  ```cheng
  fn myopen(f: out File; name: string): bool =
    f = default(File)
    result = open(f, name)
  ```

While it is usually the better style to use the return type in order to return results API and ABI
considerations might make this infeasible. Like for `var T` Cheng maps `out T` to a hidden pointer.
For example POSIX's `stat` routine can be wrapped as:

  ```cheng
  fn stat*(a1: cstring, a2: out Stat): cint {.importc, header: "<sys/stat.h>".}
  ```
...
parameter:
  ...
  ```cheng
  fn p(x: out int; y: out string; cond: bool) =
    x = 4
    if cond:
      y = "abc"
...
current module:
  ...
  ```cheng
  fn exportedEcho*(s: string) = echo s
...
This dot-call syntax is not restricted to objects, it can be used
to supply any type of first argument for fns:
...
Another way to look at the dot-call syntax is that it provides the missing
postfix notation.

The dot-call syntax conflicts with explicit generic instantiations:
`p[T](x)` cannot be written as `x.p[T]` because `x.p[T]` is always
parsed as `(x.p)[T]`.

See also: [Limitations of the dot-call syntax].
...
`echo(f(1, f(2)))`. The dot-call syntax may be used to provide one
more argument in this case:

  ```cheng
  fn optarg(x: int, y: int = 0): int = x + y
  fn singlearg(x: int): int = 20*x

  echo optarg 1, " ", singlearg 2  # prints "1 40"

  let fail = optarg 1, optarg 8   # Wrong. Too many arguments for a command call
  let x = optarg(1, optarg 8)  # traditional fn call with 2 arguments
  let y = 1.optarg optarg 8    # same thing as above, w/o the parenthesis
  assert x == y
  ```

The command invocation syntax also can't have complex expressions as arguments.
For example: [anonymous fns], `if`,
...
Unnamed fns can be used as lambda expressions to pass into other
fns:

  ```cheng
  var cities = @["Frankfurt", "Tokyo", "New York", "Kyiv"]

  cities.sort(fn (x, y: string): int =
    cmp(x.len, y.len))
  ```


Fns as expressions can appear both as nested fns and inside top-level
executable code.
...
regular fn expression, the `do` keyword can be used to pass
anonymous fns to routines:
  ...
  # Less parentheses using the dot-call plus command syntax:
  cities = cities.map do (x: string) -> string:
    "City of " + x
  ```

`do` is written after the parentheses enclosing the regular fn parameters.
The fn expression represented by the `do` block is appended to the routine
...
`do` with a parameter list or pragma list corresponds to an anonymous `fn`,
...
A closure is a fn that captures variables from its surrounding scope. In Cheng, closures are created by declaring a fn inside another fn or block:
...
```cheng
fn createAdder(x: int): fn(y: int): int =
  result = fn(y: int): int =
    return x + y
...
```cheng
fn createAccumulator(): fn(x: int): int =
  var total = 0
  result = fn(x: int): int =
    total += x
    return total
...
A `func` is currently simply a different spelling for a `fn`. This will be changed in the future. A `func` will be strict about what it can do.



## Lifetime-tracking hooks

A type bound operator is a `fn` or `func` whose name starts with `=` but isn't an operator
...
Type bound operations are the foundation for Cheng's *scope based memory management*.
...
With the language mechanisms described here, a custom seq could be written as:
  ...
  fn `=copy`*[T](a: var myseq[T]; b: myseq[T]) =
    ...
    a.data = nil
    if b.data != nil:
      a.data = typeof(a.data)(alloc(a.cap * sizeof(T)))
      for i in 0..<a.len:
        a.data[i] = b.data[i]

  fn `=dup`*[T](a: myseq[T]): myseq[T] {.nodestroy.} =
    ...
    if a.data != nil:
      result.data = typeof(result.data)(alloc(result.cap * sizeof(T)))
      for i in 0..<result.len:
        result.data[i] = `=dup`(a.data[i])

  fn `=sink`*[T](a: var myseq[T]; b: myseq[T]) =
  ...
  fn add*[T](x: var myseq[T]; y: sink T) =
    if x.len >= x.cap:
      x.cap = max(x.len + 1, x.cap * 2)
      x.data = typeof(x.data)(realloc(x.data, x.cap * sizeof(T)))
    x.data[x.len] = y
    inc x.len

  fn `[]`*[T](x: myseq[T]; i: Natural): var T =
    assert i < x.len
    result = x.data[i]

  fn `[]=`*[T](x: var myseq[T]; i: Natural; y: sink T) =
    assert i < x.len
    x.data[i] = y

  fn createSeq*[T](elems: openArray[T]): myseq[T] =
    result = myseq[T](
      len: elems.len,
      cap: elems.len,
      data: typeof(result.data)(alloc(result.cap * sizeof(T))))
    for i in 0..<result.len: result.data[i] = elems[i]

  fn len*[T](x: myseq[T]): int {.inline.} = x.len
  ```
...
The ordinary assignment in Cheng conceptually copies the values. The `=copy` hook
...
The `=copy` fn can be marked with the `{.error.}` pragma. Then any assignment
that otherwise would lead to a copy is prevented at compile-time. This looks like:

  ```cheng
  fn `=copy`(dest: var T; source: T) {.error.}
  ```
...
The prototype of this hook for a type `T` needs to be:
  ...
  ```cheng
  fn `=trace`(dest: var T; env: pointer)
  ```
...
For example:
  ...
  ```cheng
  fn main(normalParam: string; sinkParam: sink string) =
    var x = "abc"
    # valid:
...
* Objects that contain pointers that point to the same object are not supported
  by Cheng's model. Otherwise swapped objects would end up in an inconsistent state.
...
A sink parameter
*may* be consumed once in the fn's body but doesn't have to be consumed at all.
The reason for this is that signatures
like `fn put(t: var Table; k: sink Key, v: sink Value)` should be possible
...
`x = select(rand() < 0.5, x, y)` here:
  ...
  ```cheng
  fn select(cond: bool; a, b: sink string): string =
    if cond:
...
Is transformed into:
  ...
  ```cheng
  fn select(cond: bool; a, b: sink string): string {.nodestroy.} =
    if cond:
      result = a
      `=wasMoved`(a)
    else:
...
The abstraction fails for `&x` because whether the address is going to be used for mutations is unknown.
...
The following built-in fns cannot be overloaded for reasons of implementation
...
Functions use static dispatch by default. Dynamic dispatch (virtual call)
is supported for reference-typed receivers. Declarations still use `fn`;
there is no `method` keyword.
  ...
  fn eval(e: Expression): int =
    # override base fn
    quit "abstract fn without override"

  fn eval(e: Literal): int = return e.x

  fn eval(e: PlusExpr): int =
    # watch out: relies on dynamic binding
    result = eval(e.a) + eval(e.b)

  fn newLit(x: int): Literal = Literal(x: x)
  fn newPlus(a, b: Expression): PlusExpr = PlusExpr(a: a, b: b)

  echo eval(newPlus(newPlus(newLit(1), newLit(2)), newLit(4)))
  ```

In the example the constructors `newLit` and `newPlus` use static binding,
but `eval` requires dynamic binding.


### Inhibit dynamic dispatch via procCall

Dynamic dispatch can be inhibited via the builtin `system.procCall`:idx:.
This is somewhat comparable to the `super`:idx: keyword that traditional OOP
languages offer.

  ```cheng  test = "cheng c $1"
  type
...
An iterator is similar to a fn, except that it can be called in the
...
fn processFile(filename: string) =
  var f: File
  if open(f, filename):
    defer: close(f)  # Will be called when fn exits
    # Process the file...
    if someError():
...
The `defer` statement can be used in any block scope (fns, methods, iterators, etc.). Multiple `defer` statements in the same scope are executed in reverse order (last in, first out):
...
`Self` stands for the currently defined concept itself. It is used to avoid a recursion, `fn <=(a, b: Comparable): bool` is invalid.
...
Syntactically a concept consists of a list of fn and iterator declarations.
...
type
  Indexable[T] = concept # has a T, a collection
    fn `[]`(a: Self; index: int): var T # we need to describe how to infer 'T'
    # and then we can use the 'T' and it must match:
    fn `[]=`(a: var Self; index: int; value: T)
    fn len(a: Self): int
```
...
Generics are a means to parametrize fns, iterators or types with `type parameters`:idx:. Depending on the context, the brackets are used either to introduce type parameters or to instantiate a generic fn, iterator, or type.

The following example describes a generic fn `find` that can be used to look for an element in any container that fulfills the `Findable` constraints:
...
fn find[T](x: Findable[T]; elem: T): int =
  var i = 0
  for a in items(x):
    if a == elem: return i
    inc i
...
```cheng
fn processUntyped[T](x: T): string {.untyped.} =
  when T is string:
    "String: " + x
  elif T is int:
    "Integer: " + $x
...
mechanism that operates on Cheng's abstract syntax trees. It is processed in
the semantic pass of the compiler.

The syntax to *invoke* a template is the same as calling a fn.
...
Example:
  ...
  assert(5 != 6) # the compiler rewrites that to: assert(!(5 == 6))
  ```
...
passed to the `body` parameter and type checking in Cheng implies symbol lookups.
...
If `typeof` is used to determine the result type of a fn/iterator
...
passing `typeOfProc` as the second argument to `typeof`:
  ...
  ```cheng  test = "cheng c $1"
  iterator split(s: string): string = ""
  fn split(s: string): seq[string] = @[]
...
After the `import` keyword, a single normalized module path follows. Use `/`
separators between identifiers and put each module on its own line. `as` can
be used to introduce an alias:

  ```cheng
  import std/strutils
  import cheng/stdlib/bootstrap/std/sequtils as qu
  ```

`except`, `from import`, and multi-import lines are not supported.

The `import` statement is only allowed at the top level.

String/relative/absolute path imports are not supported; use normalized module paths like `import cheng/<pkg>/...`.


### Include statement

The `include` statement has been removed. Use `import` to reference modules.
...
A module alias can be introduced via the `as` keyword, after which the original module name is inaccessible:
  ...
  ```cheng
  import cheng/stdlib/bootstrap/std/strutils as su
  import cheng/stdlib/bootstrap/std/sequtils as qu

  echo su.format("$1", "lalelu")
  ```

Normalized module paths use `/` separators; dot-separated imports and string paths are removed:

  ```cheng
  import cheng/stdlib/bootstrap/std/[os,strutils]
  ```

Note that the module name is still `strutils` and not `cheng/stdlib/bootstrap/std/strutils`,
thus one **cannot** do:

  ```cheng
  import cheng/stdlib/bootstrap/std/strutils
  echo cheng/stdlib/bootstrap/std/strutils.toUpperAscii("abc")
  ```
...
The `from import` statement has been removed. Use `import` and fully-qualified
names instead.
...
modules don't need to import a module's dependencies:
  ...
  fn `$`*(x: MyObject): string = "my object"
  ```
...
fn or iterator overloading purposes.
...
* Using the identifier in a context where the compiler can infer the type
  ...
  foo("abc") # A: abc
  foo(123) # B: 123
  let inferred: fn (x: string) = foo
  foo("def") # A: def

  write(stdout, x) # error: x is ambiguous
  write(stdout, A.x) # no error: qualifier used

  fn bar(a: int): int = a + 1
...
### noreturn pragma
The `noreturn` pragma is used to mark a fn that never returns.
...
annotate a symbol (like an iterator or fn). The *usage* of the symbol then
triggers a static error. This is especially useful to rule out copy operations:

  ```cheng
  fn `=copy`(dest: var MyType, source: MyType) {.error.}
  ```



### nodecl pragma
The `nodecl` pragma can be applied to almost any symbol (variable, fn,
type, etc.) and is sometimes useful for interoperability with C:

  ```cheng
  var
    EACCES {.importc, nodecl.}: cint # pretend EACCES was a variable, as
                                     # Cheng does not know its value
  ```
...
and instead, the generated code should contain an `#include`:c:\:
  ...
  type
    PFile {.importc: "FILE*", header: "<stdio.h>".} = distinct pointer
      # import C's FILE* type; Cheng will treat it as a new pointer type
  ```

The `header` pragma always expects a string constant. The string constant
contains the header file: As usual for C, a system header file is enclosed
in angle brackets: `<>`:c:. If no angle brackets are given, Cheng
...
The `bycopy` pragma can be applied to an object or a fn param. It instructs the compiler to pass the type by value to fns:
...
The `byref` pragma can be applied to an object or a fn param.
When applied to a type it instructs the compiler to pass the type by reference
(hidden pointer) to fns. When applied to a param it will take precedence, even
if the the type was marked as `bycopy`. When an `importc` type has a `byref` pragma or
parameters are marked as `byref` in an `importc` fn, these params translate to pointers.


### Varargs pragma
The `varargs` pragma can be applied to fns, fn types and templates.
...
Plugins are a way to extend the language with new functionality. A plugin is a template that lacks a body. Instead it has a `{.plugin.}` pragma listing the Cheng program that implements the plugin.
...
In "deps/mplugin1.cheng" there is the implementation:

```cheng
import chengonyplugins
...
**Note that plugins are compiled with Cheng 2, not Cheng as Cheng is not considered stable enough.**
...
will search for `myplugin.cheng` in `dirA` (and in `std`).
...
Module plugins can also be attached to a nominal type (or a generic type that becomes a nominal type after instantiation). These plugins are invoked for every module that uses the type. This mechanism can replace Cheng's "term rewriting macros":
...
fn `*=`(x: var Matrix; y: Matrix) = ...
fn `+=`(x: var Matrix; y: Matrix) = ...
fn `-=`(x: var Matrix; y: Matrix) = ...

fn `*`(x, y: Matrix): Matrix =
  result = x; result *= y
fn `+`(x, y: Matrix): Matrix =
  result = x; result += y
fn `-`(x, y: Matrix): Matrix =
...
The `import (path/...) {.plugin: "...".}` form has been removed. Use a normal
`import` to load generated modules, and invoke plugins via template or module
pragmas instead.
