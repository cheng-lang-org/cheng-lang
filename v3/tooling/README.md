...
- `v3/tooling/run_v3_stage23_libp2p_smokes.sh` 会直接拿 `artifacts/v3_bootstrap/cheng.stage2` 和 `cheng.stage3` 真编真跑当前已经闭合的 `libp2p_core/libp2p_host/libp2p_tcp/libp2p_overlay/libp2p_protocols/chain_node_libp2p` 主线；现在它只额外挂一条 `fixed256_curve25519_smoke` 当 `TLS13 X25519` 的定点底座前哨
- `v3/src/tests/libp2p_quic_tls_smoke.cheng` 继续保留，但当前不在 `stage2/stage3` 主 gate；真实 `QUIC/TLS` 闭包还没收完，不能冒充成已闭合主线
- `v3/tooling/run_v3_host_smokes.sh` 会用当前 host compiler 真编真跑 `ref10_ashr/fixed256_curve25519/fixedbytes32_seq_index/program_selfhost/bft/overlay/csg/consensus/chain_node/pubsub/location_proof/chain_codec/anti_entropy/lsmr*` 和 `udp_importc_smoke`；也支持 `sh run_v3_host_smokes.sh smoke_a smoke_b` 这种定向 smoke；可用 `CHENG_V3_SMOKE_COMPILER=<path>` 切编译器入口
