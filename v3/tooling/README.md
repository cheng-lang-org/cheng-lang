...
- `v3/tooling/cheng_v3.sh` 现在除了 `help / bootstrap-bridge / print-bootstrap / r2c-react-v3` 这几个特殊入口外，其他命令都按“需 contract”或“plain stage3”两组统一分派，不再维护一长串重复 case
- `v3/tooling/bootstrap_bridge_v3.sh` 现在只保留最小冷启动职责：优先复用现成 `cheng.stage3`，其次复用现成 `cheng.stage0`，最后才回 C seed 临时 runner；真正的 freshness 和 bootstrap 物化都由 `stage0/stage3 bootstrap-bridge` 本体负责
- `v3/tooling/build_backend_driver_v3.sh` 会先刷新 `bootstrap.env`，然后直接把 fresh `cheng.stage3` 的 live `build-backend-driver` 命令转发到 `artifacts/v3_backend_driver/cheng`
...
- `v3/tooling/cheng_v3.sh verify-debug-runtime` 现在不只看 `panic/bounds/signal` 运行输出，还会硬查 ordinary build report 里的 `provider_source_paths` 必须直指 `src/runtime/native/system_helpers_debug_trace_profile.c`，并用内建 `print-object` 同时钉死 `runtime_debug_runtime_v3.o` 的导出符号面、Darwin 宿主未定义符号白名单，以及 Linux AArch64 `nolibc` runtime object 的 syscall 依赖白名单
...
- `bootstrap_bridge_v3.sh` 现在分三层冷启动：`stage3` 全新则直接退出；`stage3` 和当前 seed 同代时直接让 fresh `cheng.stage3 bootstrap-bridge` 在锁内重编；如果 `stage3` 不在但 `cheng.stage0` 仍和当前 seed 同代，会优先直接复用 `cheng.stage0 bootstrap-bridge`；只有 seed 自己变了才回 C seed 临时 runner
