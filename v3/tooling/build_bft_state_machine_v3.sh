...
if [ ! -x "$driver" ]; then
  echo "v3 bft_state_machine: missing backend driver: $driver" >&2
  exit 1
fi

if [ ! -f "$src" ]; then
  echo "v3 bft_state_machine: missing source: $src" >&2
  exit 1
fi
...
if [ "$build_rc" -ne 0 ]; then
  echo "v3 bft_state_machine: backend driver failed rc=$build_rc log=$compile_log" >&2
  tail -n 80 "$compile_log" >&2 || true
  exit 1
fi

if [ ! -x "$out_bin" ]; then
  echo "v3 bft_state_machine: backend driver returned success but no executable was produced: $out_bin" >&2
  exit 1
fi

if ! "$out_bin" >"$run_log" 2>&1; then
  echo "v3 bft_state_machine: self-test failed log=$run_log" >&2
