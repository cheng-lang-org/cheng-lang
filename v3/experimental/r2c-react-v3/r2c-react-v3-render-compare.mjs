...
function main() {
  ...
  const nativeGuiBundleHelper = path.join(scriptDir, 'r2c-react-v3-native-gui-bundle.mjs');
  const nativeGuiRunHelper = path.join(scriptDir, 'r2c-react-v3-native-gui-run.mjs');
  const truthRuntimeHelper = path.join(scriptDir, 'r2c-react-v3-truth-runtime.mjs');
  const outDir = path.resolve(args.outDir || path.join(repo, 'build', 'r2c_react_v3_render_compare', route));
