# Cheng 重写 codex-rs 第一阶段方案

  ## Summary

  目标是在 /Users/lbcheng/cheng-lang/codex 内，用 Cheng 重建一套可日常使用的 Codex 首发产品，第一阶段同时覆盖 core + exec + cli
  ...
  - 在 codex 下建立独立 Cheng 包，不混进现有 `src/core/tooling` 和 `src/apps` 主线；目录固定拆成：
  ...
  - 复用现有主线能力，不重复造轮子：
    命令行解析走 std/cmdline；
    本地进程/文件/路径能力尽量复用 `src/core/tooling`、`src/std`、`src/core/runtime` 现有实现；
  ...
  - 新增四个产品入口：
    codex/exec/main.cheng
    codex/cli/main.cheng
    codex/tui/main.cheng
    codex/core/*.cheng
  ...
  - 现有 runtime、tooling、命令执行和文件系统能力足够承载首发版本；若发现硬缺口，优先补 Cheng 底层能力，不允许塞临时脚本绕过。
  - `codex` 作为独立包推进，避免污染当前 `src/` 既有编译器和链路主线。
