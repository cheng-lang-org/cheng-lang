...
- 当前 `backend_driver_dispatch_min.cheng` 已可被 cold `system-link-exec` 消费并生成 Mach-O：`backend_driver_dispatch_min_probe` 编译成功，最新报告 `cold_compile_elapsed_ms=33.410`。
- 生成物 `backend_driver_dispatch_min_probe system-link-exec` 已越过 `ccsg.BuildCompilerCsgInto`，当前在 `lower.BuildLoweringPlanStubFromCompilerCsg` 结构化失败：exit `2`，stderr 写出 `cold runtime unsupported: lower.BuildLoweringPlanStubFromCompilerCsg`，不再 SIGTRAP。
- 当前硬边界：`ccsg.BuildCompilerCsgInto` 已能返回最小结构化 CSG 控制对象；`lower.BuildLoweringPlanStubFromCompilerCsg` 已改为结构化 false+error；`pobj/direct` 重后端路径仍只做运行时 `brk` 暴露，不伪造成功。
- 新增观测：冷报告输出最大函数帧，当前 `cold_max_frame_size=22032`，最大函数 `BackendDriverDispatchMinAppendPrimaryPlanReport`；`AfterSourceBundle=784`、`AfterCsg=1600`，说明 lowering report 缺失不是这两个函数的大栈帧直接导致，但 ARM64 大帧/大偏移编码仍是后续必须补齐的硬债。
- 新暴露缺口：lowering 失败路径的 report sidecar 没写出；`WriteTextFile(root,path,text)` 已改为按 root 解析 path，且对象字段写新增越界硬检查，但 report 缺失仍未修复。下一步先修 `AfterCsg` 层错误报告桥/参数 ABI，再把 `lowering_plan -> primary_object_plan -> direct_object_emit` 的 Cheng body 闭包纳入冷端，或逐个替换为严格等价 materializer。
