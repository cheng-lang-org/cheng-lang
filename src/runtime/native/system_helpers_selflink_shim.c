...
#define CHENG_SHIM_MAYBE_UNUSED __attribute__((unused))
#else
#define CHENG_SHIM_WEAK
#define CHENG_SHIM_MAYBE_UNUSED
#endif
...
CHENG_SHIM_WEAK int32_t driver_emit_obj_from_module_default_impl(void *module, const char *target,
                                                                 const char *path);
CHENG_SHIM_WEAK int32_t driver_export_emit_obj_from_module_default_impl(void *module,
                                                                        const char *target,
                                                                        const char *path);
CHENG_SHIM_WEAK int32_t driver_build_emit_obj_from_file_stage1_target_impl(const char *input_path,
                                                                           const char *target,
                                                                           const char *output_path);
CHENG_SHIM_WEAK int32_t driver_export_build_emit_obj_from_file_stage1_target_impl(
...
CHENG_SHIM_WEAK void *driver_buildModuleFromFileStage1TargetRetained(const char *path,
                                                                     const char *target);
CHENG_SHIM_WEAK void *driver_export_buildModuleFromFileStage1TargetRetained(const char *path,
                                                                            const char *target);
CHENG_SHIM_WEAK void *driver_seed_bridge_buildModuleFromFileStage1TargetRetained_impl(
    const char *path, const char *target);
CHENG_SHIM_WEAK int32_t driver_seed_bridge_emit_obj_from_module_default_impl(
...
CHENG_SHIM_WEAK void *driver_c_proof_buildModuleFromFileStage1TargetRetained(
...
CHENG_SHIM_WEAK void *driver_c_proof_buildModuleFromFileStage1Retained(
    const char *input_path) {
  return driver_c_proof_buildModuleFromFileStage1TargetRetained(input_path, "");
}

CHENG_SHIM_WEAK int32_t driver_c_proof_build_emit_obj_from_file_stage1_target_impl(
    const char *input_path, const char *target, const char *output_path) {
  driver_c_maybe_run_self_global_init();
...
CHENG_SHIM_WEAK void *driver_c_seed_bridge_buildModuleFromFileStage1TargetRetained(
    const char *input_path, const char *target) {
  driver_c_maybe_run_self_global_init();
  if (input_path == NULL || input_path[0] == '\0') return NULL;
  if (target == NULL) target = "";
...
CHENG_SHIM_WEAK int32_t driver_c_seed_bridge_emit_obj_from_module_default_impl(
    void *module, const char *target, const char *output_path) {
  driver_c_maybe_run_self_global_init();
...
CHENG_SHIM_WEAK int32_t driver_c_seed_bridge_build_emit_obj_from_file_stage1_target_impl(
    const char *input_path, const char *target, const char *output_path) {
  driver_c_maybe_run_self_global_init();
  if (input_path == NULL || input_path[0] == '\0') return -30;
  if (target == NULL || target[0] == '\0') return -31;
...
static CHENG_SHIM_MAYBE_UNUSED int driver_c_arg_is_help(const char *arg) {
...
static CHENG_SHIM_MAYBE_UNUSED int driver_c_runtime_path_contains(const char *path, const char *needle) {
  return path != NULL && needle != NULL && strstr(path, needle) != NULL;
}

static CHENG_SHIM_MAYBE_UNUSED int driver_c_runtime_resolve_self_path(char *out, size_t out_cap) {
  char raw[PATH_MAX];
  char *resolved = NULL;
...
CHENG_SHIM_WEAK int32_t cheng_cstrlen(const char *s) {
  return cheng_strlen((char *)s);
}

CHENG_SHIM_WEAK int32_t min(int32_t a, int32_t b) {
  return (a < b) ? a : b;
}

CHENG_SHIM_WEAK ChengStrBridge load(const ChengStrBridge *p) {
  ChengStrBridge out;
...
CHENG_SHIM_WEAK void cheng_str_bridge_load_into(void *out_raw, const ChengStrBridge *p) {
...
CHENG_SHIM_WEAK void store(ChengStrBridge *p, ChengStrBridge val) {
  if (p == NULL) {
    return;
  }
  *p = val;
...
CHENG_SHIM_WEAK ChengStrBridge driver_c_str_slice_bridge(ChengStrBridge s, int32_t start, int32_t count) {
...
CHENG_SHIM_WEAK void cheng_error_info_bridge_ok(void *out_raw) {
  if (!out_raw) return;
...
CHENG_SHIM_WEAK void cheng_error_info_bridge_new(void *out_raw, ChengStrBridge msg) {
  if (!out_raw) return;
  ChengErrorInfoBridgeCompat *out = (ChengErrorInfoBridgeCompat *)out_raw;
  out->code = 0;
  out->msg = msg;
...
CHENG_SHIM_WEAK void cheng_error_info_bridge_copy(void *out_raw, int32_t code, ChengStrBridge msg) {
  if (!out_raw) return;
  ChengErrorInfoBridgeCompat *out = (ChengErrorInfoBridgeCompat *)out_raw;
  out->code = code;
  out->msg = msg;
...
CHENG_SHIM_WEAK void cheng_error_info_bridge_copy_from(void *out_raw, ChengErrorInfoBridgeCompat src) {
  if (!out_raw) return;
  ChengErrorInfoBridgeCompat *out = (ChengErrorInfoBridgeCompat *)out_raw;
  *out = src;
}

CHENG_SHIM_WEAK int32_t cheng_error_info_bridge_code(ChengErrorInfoBridgeCompat src) {
  return src.code;
}

CHENG_SHIM_WEAK void cheng_error_info_bridge_msg_into(void *out_raw, ChengErrorInfoBridgeCompat src) {
  if (!out_raw) return;
  ChengStrBridge *out = (ChengStrBridge *)out_raw;
  *out = src.msg;
}

CHENG_SHIM_WEAK ChengStrBridge cheng_error_info_bridge_msg(ChengErrorInfoBridgeCompat src) {
  return src.msg;
}
...
void *cheng_seq_set_grow(void *seq_ptr, int32_t idx, int32_t elem_size) {
  int32_t actual_elem_size = cheng_seq_elem_size_compat_get(seq_ptr, elem_size);
  if (!seq_ptr || actual_elem_size <= 0) return cheng_seq_get(NULL, 0, idx, actual_elem_size);
  ChengSeqHeader *seq = (ChengSeqHeader *)seq_ptr;
  cheng_seq_sanitize(seq);
  if (idx < 0) return cheng_seq_get(seq->buffer, seq->len, idx, actual_elem_size);
  int32_t need = idx + 1;
  if (need > seq->cap || seq->buffer == NULL) {
    int32_t new_cap = cheng_next_cap(seq->cap, need);
    if (new_cap <= 0) return cheng_seq_get(seq->buffer, seq->len, idx, actual_elem_size);
    size_t old_bytes = (size_t)(seq->cap > 0 ? seq->cap : 0) * (size_t)actual_elem_size;
    size_t new_bytes = (size_t)new_cap * (size_t)actual_elem_size;
    void *new_buf = c_realloc(seq->buffer, (int64_t)new_bytes);
    if (!new_buf) return cheng_seq_get(seq->buffer, seq->len, idx, actual_elem_size);
    if (new_bytes > old_bytes) {
  ...
  return cheng_seq_get(seq->buffer, seq->len, idx, actual_elem_size);
}
