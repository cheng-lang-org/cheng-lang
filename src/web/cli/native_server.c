...
#ifndef CHENG_SERVER_HANDLE
#define CHENG_SERVER_HANDLE server_handle
#endif

extern void *CHENG_SERVER_HANDLE(void *req_ptr, int req_len, int *out_len);
...
static ServerConfig load_config(void) {
  ServerConfig cfg;
  cfg.max_header = parse_int(getenv("CHENG_WEB_SERVER_MAX_HEADER"), 16384);
  cfg.max_body = parse_int(getenv("CHENG_WEB_SERVER_MAX_BODY"), 1024 * 1024);
  cfg.timeout_ms = parse_int(getenv("CHENG_WEB_SERVER_TIMEOUT_MS"), 5000);
  return cfg;
}
...
static void run_server(const char *host, int port, ServerConfig cfg) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      int out_len = 0;
      void *out = CHENG_SERVER_HANDLE(req, req_len, &out_len);
      if (out && out_len > 0) {
...
int cheng_native_server_main(int argc, char **argv) {
  const char *host = getenv("CHENG_WEB_SERVER_HOST");
  int port = parse_port(getenv("CHENG_WEB_SERVER_PORT"));
