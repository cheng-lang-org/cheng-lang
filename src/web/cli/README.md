...
- `web.cheng` provides a single entrypoint for `dev/build/preview/route-manifest`.
  - build via `sh src/tooling/tooling_exec.sh chengc cheng/web/cli/web.cheng --name:cheng_web`.
...
- `dev_server.cheng` runs a minimal static server with polling reload and optional build command (default build-exts: `.cwc`); can auto-run route manifest via `WEB_MANIFEST_ROOT` (aliases: `WEB_ROUTES_ROOT` / `--routes-root`).
  ...
  - build via `sh src/tooling/tooling_exec.sh web_build_native_server --entry:cheng/web/cli/dev_server_main.cheng --name:cheng_dev_server`.
- `preview_server.cheng` runs a static server for built assets.
  - supports `--spa` / `WEB_SPA` to serve `index.html` for client-side routes.
  - build via `sh src/tooling/tooling_exec.sh web_build_native_server --entry:cheng/web/cli/preview_server_main.cheng --name:cheng_preview_server`.
- `route_manifest.cheng` scans a routes folder and emits `routes_manifest.cheng` for file-based routing.
  - build via `sh src/tooling/tooling_exec.sh chengc cheng/web/cli/route_manifest.cheng --name:cheng_route_manifest`.
...
- `web_build_native_server` (tooling id) compiles `server_main.cheng` and links `native_server.c` into a runnable binary.
