...
- Run: `CHENG_WEB_ROOT=cheng/web/examples ./cheng_dev_server --host:127.0.0.1 --port:5173`
- Optional: `CHENG_WEB_BUILD="<your build command>"` to rebuild wasm on change.
  - Or build `src/tooling/chengc.sh cheng/web/cli/web.cheng --name:cheng_web` then run `CHENG_WEB_ROOT=cheng/web/examples ./cheng_web dev --host:127.0.0.1 --port:5173`.

Build (compile .cwc):
- `src/tooling/chengc.sh cheng/web/cli/web.cheng --name:cheng_web`
- `CHENG_WEB_ROOT=cheng/web/examples ./cheng_web build --out:cheng/web/examples/dist --map` (copies common assets + map)
...
- Run: `CHENG_WEB_ROOT=cheng/web/examples ./cheng_preview_server --host:127.0.0.1 --port:4173`
  - Or run `CHENG_WEB_ROOT=cheng/web/examples ./cheng_web preview --host:127.0.0.1 --port:4173`.
...
- Compile `router_app.cheng` to `client.wasm` to try routing.
  - Alternatively, set `CHENG_WEB_MANIFEST_ROOT=cheng/web/examples/routes` and run the dev server to auto-generate.
