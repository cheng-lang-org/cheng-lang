...
usage() {
  cat <<'EOF'
Usage:
  src/tooling/verify_c_backend_importc_ffi.sh [--example:<path>] [--impl:<path>] [--name:<exe>] [--out-dir:<dir>]

Notes:
  - Verifies importc aggregate return + packed/align mapping in direct C backend.
  - Builds the Cheng module with the C frontend, then links with the C impl.
EOF
}

example="${CHENG_C_FFI_EXAMPLE:-examples/c_backend_importc_ffi.cheng}"
impl="${CHENG_C_FFI_IMPL:-examples/c_backend_importc_ffi.c}"
name="${CHENG_C_FFI_NAME:-c_backend_importc_ffi}"
out_dir="${CHENG_C_FFI_OUT_DIR:-chengcache/c_backend_importc_ffi}"
...
echo "verify_c_backend_importc_ffi ok"
