...
Env:
  CHENG_BOOTSTRAP_UPDATE_SEED=1  Copy artifacts/backend_selfhost_self_obj/cheng.stage2
...
if [ "$seed_override" != "" ]; then
  export CHENG_SELF_OBJ_BOOTSTRAP_STAGE0="$seed_override"
fi
...
if [ "${CHENG_BOOTSTRAP_UPDATE_SEED:-0}" = "1" ]; then
