...
usage() {
  cat <<'EOF'
Usage:
  src/tooling/verify_backend_rawptr_contract.sh [--baseline:<path>] [--doc:<path>] [--formal-doc:<path>] [--tooling-doc:<path>]

Notes:
  - Verifies RPSPAR-01 raw pointer safety contract freeze baseline.
  - Regenerate baseline with: sh src/tooling/build_backend_rawptr_contract.sh
EOF
}
...
sh src/tooling/build_backend_rawptr_contract.sh \
  --doc:"$doc" \
...
if ! rg -q 'run_step "backend.rawptr_contract"' src/tooling/verify_backend_closedloop.sh; then
  closedloop_gate_ok="0"
fi
if ! rg -q 'run_step "backend.rawptr_surface_forbid"' src/tooling/verify_backend_closedloop.sh; then
  closedloop_gate_ok="0"
fi
if ! rg -q 'run_required "backend.rawptr_contract"' src/tooling/backend_prod_closure.sh; then
  prod_closure_gate_ok="0"
fi
if ! rg -q 'run_required "backend.rawptr_surface_forbid"' src/tooling/backend_prod_closure.sh; then
  prod_closure_gate_ok="0"
fi
...
if [ "$status" != "ok" ]; then
  ...
  echo "  fix: sh src/tooling/build_backend_rawptr_contract.sh --doc:$doc --formal-doc:$formal_doc --tooling-doc:$tooling_doc --out:$baseline" 1>&2
  exit 1
fi
