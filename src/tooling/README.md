...
- `.cap.env` 侧车由正在运行的 `artifacts/tooling_cmd/cheng_tooling` 可执行文件生成，不是直接从源码现算；如果 binary 陈旧，侧车仍可能保留旧的 `source_count=8` 签名。遇到这种情况应先重建 canonical tooling，再决定是否需要刷新 seed/driver。
...
- `build_backend_driver` fast 轨默认 `BACKEND_BUILD_DRIVER_MULTI=0`、`BACKEND_BUILD_DRIVER_INCREMENTAL=1`，并默认开启 `BACKEND_BUILD_DRIVER_QUICK_SHARED_CACHE=1`，允许同一份源码/stage0/参数在不同 `--out` 之间复用 quick 产物；quick/shared stamp 现在额外绑定 `source_capability_signature`，覆盖与 stage0 capability preflight 相同的 27 文件源列表（`std/cmdline`、stage1 token/lexer/parser/frontend_lib/diagnostics/semantics/ownership/monomorphize/type_syntax_lowering/c_profile、UIR core/builder/codegen、machine core/select、obj writer/linker、`backend_driver`）。seal 轨默认 `BACKEND_BUILD_DRIVER_REBUILD_MULTI=0`、`BACKEND_BUILD_DRIVER_REBUILD_INCREMENTAL=0`。需要并行时可显式覆盖 jobs/multi。
