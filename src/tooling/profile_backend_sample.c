...
static void usage(void) {
  fprintf(stderr,
          ...
          "\n"
          "Presets:\n"
          "  selfhost-cold  env CHENG_CLEAN_CHENG_LOCAL=0 CHENG_SELF_OBJ_BOOTSTRAP_REUSE=0\n"
          "                 CHENG_SELF_OBJ_BOOTSTRAP_SESSION=sample_cold\n"
          "                 sh src/tooling/verify_backend_selfhost_bootstrap_self_obj.sh\n"
          "\n"
          "  fullchain-cold env CHENG_CLEAN_CHENG_LOCAL=0 CHENG_FULLCHAIN_REUSE=0\n"
          "                 CHENG_FULLCHAIN_TOOL_JOBS=3\n"
          "                 sh src/tooling/verify_fullchain_bootstrap.sh\n"
          "\n"
          "  closure-cold   env CHENG_CLEAN_CHENG_LOCAL=0 CHENG_BACKEND_PROD_SELFHOST_REUSE=0\n"
          "                 CHENG_FULLCHAIN_REUSE=0 CHENG_FULLCHAIN_TOOL_JOBS=3\n"
...
static int pick_preset_command(const char *preset, char *const **out_argv, const char **out_default_attach) {
  static char *const selfhost[] = {
      ...
      "CHENG_SELF_OBJ_BOOTSTRAP_REUSE=0",
      "CHENG_SELF_OBJ_BOOTSTRAP_SESSION=sample_cold",
      "sh",
  ...
  static char *const closure[] = {
      "env",
      "CHENG_CLEAN_CHENG_LOCAL=0",
      "CHENG_BACKEND_PROD_SELFHOST_REUSE=0",
      "CHENG_FULLCHAIN_REUSE=0",
      "CHENG_FULLCHAIN_TOOL_JOBS=3",
      "sh",
...
int main(int argc, char **argv) {
  ...
  if (strcmp(getenv("CHENG_PROFILE_SAMPLE_SKIP_OS_CHECK") == NULL ? "" : getenv("CHENG_PROFILE_SAMPLE_SKIP_OS_CHECK"), "1") != 0) {
    if (uname(&uts) != 0) {
