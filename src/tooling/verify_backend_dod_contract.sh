...
usage() {
  cat <<'EOF'
Usage:
  src/tooling/verify_backend_dod_contract.sh [--baseline:<path>] [--doc:<path>]

Notes:
  - Verifies DOPAR-01 DOD contract freeze baseline.
  - Regenerate baseline with: sh src/tooling/build_backend_dod_contract.sh
EOF
}
...
sh src/tooling/build_backend_dod_contract.sh --doc:"$doc" --out:"$generated" >/dev/null
...
if ! rg -q 'run_step "backend.dod_contract"' src/tooling/verify_backend_closedloop.sh; then
  closedloop_gate_ok="0"
fi
if ! rg -q 'run_required "backend.dod_contract"' src/tooling/backend_prod_closure.sh; then
...
if [ "$status" != "ok" ]; then
  ...
  echo "  fix: sh src/tooling/build_backend_dod_contract.sh --doc:$doc --out:$baseline" 1>&2
  exit 1
fi
