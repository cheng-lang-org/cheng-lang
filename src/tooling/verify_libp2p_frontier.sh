...
Env:
  CHENG_BACKEND_DRIVER=<path>        Explicit backend driver path
  CHENG_FRONTIER_TIMEOUT=<seconds>   Stage1 probe timeout (default: 180)
  CHENG_FRONTIER_IMPORT_TIMEOUT=<seconds> Import probe timeout (default: 20)
  CHENG_FRONTIER_TIMEOUT_DIAG=<0|1>  Capture timeout sample diag (default: 1)
  CHENG_FRONTIER_TIMEOUT_DIAG_SECONDS=<seconds> sample duration on timeout (default: 5)
  CHENG_FRONTIER_TIMEOUT_DIAG_DIR=<path> timeout diag directory (default: <logs>/timeout_diag)
  CHENG_FRONTIER_TIMEOUT_DIAG_SUMMARY=<0|1> print summary for timeout sample (default: 1)
  CHENG_FRONTIER_TIMEOUT_DIAG_SUMMARY_TOP=<n> top frames to print (default: 12)
  CHENG_FRONTIER_IMPORT_FRONTEND=<mvp|stage1> Import probe frontend (default: mvp)
...
if [ -n "${CHENG_BACKEND_DRIVER:-}" ]; then
  DRIVER="$(to_abs "$CHENG_BACKEND_DRIVER")"
else
...
MULTIFORMATS_ROOT="$(resolve_repo \
  "$ROOT/../cheng-multiformats" \
  "$HOME/cheng-multiformats" \
  "$HOME/.cheng-packages/cheng-multiformats" || true)"
...
IMPORT_TIMEOUT="${CHENG_FRONTIER_IMPORT_TIMEOUT:-20}"
IMPORT_FRONTEND="${CHENG_FRONTIER_IMPORT_FRONTEND:-mvp}"
FRONTIER_SKIP_SEM="${CHENG_FRONTIER_SKIP_SEM:-1}"
FRONTIER_SKIP_OWNERSHIP="${CHENG_FRONTIER_SKIP_OWNERSHIP:-1}"
FRONTIER_VALIDATE="${CHENG_FRONTIER_VALIDATE:-0}"
FRONTIER_GENERIC_MODE="${CHENG_FRONTIER_GENERIC_MODE:-dict}"
FRONTIER_GENERIC_SPEC_BUDGET="${CHENG_FRONTIER_GENERIC_SPEC_BUDGET:-0}"
FRONTIER_TIMEOUT_DIAG="${CHENG_FRONTIER_TIMEOUT_DIAG:-1}"
FRONTIER_TIMEOUT_DIAG_SECONDS="${CHENG_FRONTIER_TIMEOUT_DIAG_SECONDS:-5}"
FRONTIER_TIMEOUT_DIAG_DIR="${CHENG_FRONTIER_TIMEOUT_DIAG_DIR:-$LOG_DIR/timeout_diag}"
FRONTIER_TIMEOUT_DIAG_TOOL="${CHENG_FRONTIER_TIMEOUT_DIAG_TOOL:-sample}"
...
prepare_timeout_diag() {
  ...
  export CHENG_TIMEOUT_DIAG_SECONDS="$FRONTIER_TIMEOUT_DIAG_SECONDS"
  export CHENG_TIMEOUT_DIAG_TOOL="$FRONTIER_TIMEOUT_DIAG_TOOL"
}
...
add_pkg_root() {
  ...
  case ",$CHENG_PKG_ROOTS," in
    *,"$root",*) ;;
    *) CHENG_PKG_ROOTS="${CHENG_PKG_ROOTS}${CHENG_PKG_ROOTS:+,}$root" ;;
  esac
}
...
add_pkg_root "$MULTIFORMATS_ROOT"
...
if [ -z "${CHENG_PKG_ROOT:-}" ]; then
  ...
  if [ -d "$parent_guess" ]; then
    CHENG_PKG_ROOT="$parent_guess"
    export CHENG_PKG_ROOT
  fi
fi
...
run_probe() {
  ...
  run_with_timeout_labeled "$name" "$timeout_s" env \
    CHENG_MM=orc \
    CHENG_BACKEND_ALLOW_NO_MAIN="${CHENG_BACKEND_ALLOW_NO_MAIN:-1}" \
    CHENG_BACKEND_VALIDATE="$FRONTIER_VALIDATE" \
    CHENG_BACKEND_EMIT=obj \
    CHENG_BACKEND_TARGET=auto \
    CHENG_BACKEND_FRONTEND="$frontend" \
    CHENG_STAGE1_SKIP_SEM="$FRONTIER_SKIP_SEM" \
    CHENG_STAGE1_SKIP_OWNERSHIP="$FRONTIER_SKIP_OWNERSHIP" \
    CHENG_GENERIC_MODE="$FRONTIER_GENERIC_MODE" \
    CHENG_GENERIC_SPEC_BUDGET="$FRONTIER_GENERIC_SPEC_BUDGET" \
    CHENG_BACKEND_INPUT="$src" \
