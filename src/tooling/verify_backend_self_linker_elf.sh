...
if [ "$objdump" = "" ]; then
  echo "[verify_backend_self_linker_elf] skip: missing llvm-objdump" >&2
  exit 2
fi
...
target="${CHENG_BACKEND_ELF_TARGET:-aarch64-unknown-linux-gnu}"
...
if [ ! -s "$runtime_obj" ]; then
  echo "[verify_backend_self_linker_elf] missing runtime obj: $runtime_obj" >&2
  exit 1
fi
