...
if [ "$skip_ownership_effective_nonzero_count" -gt 0 ]; then
  echo "[verify_backend_dod_opt_regression] ownership proof path not engaged: stage1_ownership_fixed_0_effective!=0 count=$skip_ownership_effective_nonzero_count" 1>&2
  exit 1
fi
