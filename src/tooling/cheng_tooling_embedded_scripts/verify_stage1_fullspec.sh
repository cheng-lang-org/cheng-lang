...
envs="MM=orc"
stage1_skip_sem="${STAGE1_SEM_FIXED_0:-0}"
stage1_skip_ownership="${STAGE1_OWNERSHIP_FIXED_0:-0}"
...
if [ "$target" = "" ]; then
  case "$host_os/$host_arch" in
    ...
    *)
      if [ "$allow_skip" != "" ]; then
        echo "[skip] stage1 fullspec exe: unsupported host=$host_os/$host_arch" 1>&2
        exit 0
      fi
      echo "[Error] stage1 fullspec exe unsupported host=$host_os/$host_arch" 1>&2
      exit 2
      ;;
  esac
fi
...
if [ "$linker" = "self" ] && [ "$host_os" = "Darwin" ] && ! command -v codesign >/dev/null 2>&1; then
  if [ "$allow_skip" != "" ]; then
    echo "[skip] stage1 fullspec exe: missing codesign" 1>&2
    exit 0
  fi
  echo "[Error] stage1 fullspec exe missing codesign (required for self-linked Mach-O)" 1>&2
  exit 2
fi
