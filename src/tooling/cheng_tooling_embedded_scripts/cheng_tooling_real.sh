...
is_native_only_repo_bypass() {
  case "${1:-}" in
    build-backend-driver|build_backend_driver|verify_backend_noalias_opt|verify_backend_egraph_cost|verify_backend_dod_opt_regression)
      return 0
      ;;
  esac
  return 1
...
if [ "$root" != "" ]; then
  ...
  if [ -x "$root/artifacts/tooling_cmd/cheng_tooling.real.bin" ]; then
    primary_bin="$root/artifacts/tooling_cmd/cheng_tooling.real.bin"
  fi
...
if [ "$repo_wrapper" != "" ] && [ "$(resolve_repo_script "$root" "$first_arg" || true)" != "" ]; then
  exec env TOOLING_BIN="$primary_bin" TOOLING_SELF_BIN="$primary_bin" sh "$repo_wrapper" "$@"
fi

if [ "$repo_wrapper" != "" ] && is_list_cmd "$first_arg"; then
  exec env TOOLING_BIN="$primary_bin" TOOLING_SELF_BIN="$primary_bin" TOOLING_LIST_BIN="$core_bin" sh "$repo_wrapper" "$@"
fi

if [ "$repo_wrapper" != "" ] && [ "$first_arg" = "install" ]; then
  exec env \
    TOOLING_BIN="$primary_bin" \
    TOOLING_SELF_BIN="$primary_bin" \
    TOOLING_INSTALL_DEFAULT_BIN="$root/artifacts/tooling_cmd/cheng_tooling.real" \
