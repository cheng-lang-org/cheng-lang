...
while IFS= read -r fixture; do
  ...
  for goal in balanced latency size; do
    ...
    skip_ownership_effective="$(extract_stamp_field "$stamp_a" "stage1_skip_ownership_effective")"
...
if [ "$skip_ownership_effective_nonzero_count" -gt 0 ]; then
  echo "[verify_backend_egraph_cost] ownership proof path not engaged: stage1_skip_ownership_effective!=0 count=$skip_ownership_effective_nonzero_count" 1>&2
  exit 1
fi
