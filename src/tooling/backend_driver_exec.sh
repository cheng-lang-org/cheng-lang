...
tool=""
if [ -x "$root/artifacts/tooling_cmd/cheng_tooling.real" ]; then
  tool="$root/artifacts/tooling_cmd/cheng_tooling.real"
elif [ -x "$root/artifacts/tooling_cmd/cheng_tooling.real.bin" ]; then
  tool="$root/artifacts/tooling_cmd/cheng_tooling.real.bin"
...
timeout_s="${BACKEND_DRIVER_EXEC_TIMEOUT:-${TOOLING_COMPILE_TIMEOUT:-60}}"
case "$timeout_s" in
  ''|*[!0-9]*) timeout_s=60 ;;
esac
...
attempts="${BACKEND_DRIVER_EXEC_ATTEMPTS:-1}"
case "$attempts" in
  ''|*[!0-9]*) attempts=1 ;;
esac
...
while :; do
  ...
  if [ "$rc" -ne 0 ]; then
    if [ "$rc" -eq 124 ]; then
      echo "[backend_driver_exec] timeout ${timeout_s}s for command: $*" >&2
    fi
    ue_cleanup_best_effort
